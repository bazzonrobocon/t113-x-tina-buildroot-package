...
static int algo_read(void)
{
    ...
    /*ALOGD("[%s] line:%d fd=%d, path:%s", __func__, __LINE__, fd, g_file_path);*/
    ...
    /*ALOGD("[%s] line:%d total_bytes=%d", __func__, __LINE__, total_bytes);*/

    while (total_bytes > 0) {
        bufferlen = loop_read_size;
        if (bufferlen > (ssize_t) total_bytes)
            bufferlen = total_bytes;
        /*ALOGD("[%s] line:%d bufferlen=%ld, loop_read_size=%d", __func__, __LINE__, bufferlen, loop_read_size);    */
        ret = awrpaf_component_stream_read(com, buffer, &bufferlen);
        if (ret != 0) {
            ALOGE("[%s] line:%d component read failed:%d", __func__, __LINE__, ret);
            break;
        }
        if (bufferlen != (ssize_t) loop_read_size)
            ALOGE("[%s] line:%d ERROR, bufferlen=%zd, loop_read_size=%d", __func__, __LINE__, bufferlen, loop_read_size);
...
#define TIMESTAMP_END() \
    ...
    ALOGI("line:%d time spend: %dms", __LINE__, _ms2 - _ms1);
...
static int algo_process(void)
{
    ...
    for (i = 0; i < g_looptest_count; i++) {
        ...
        if (ret != 0)
            ALOGE("[%s] line:%d algo process failed:%d", __func__, __LINE__, ret);
        if (g_interval_ms > 0)
...
static void usage(void)
{
    ...
    printf("awrpaf_test -C audiocodec -F 0 -t 4 -i 0 -s 0 -e\n");
    ...
    printf("awrpaf_test -C audiocodec -F 1 -t 4 -s 0 -d 3 -f /tmp/awrpaf_test/test.pcm\n");
    printf("  read algo data for 3 seconds, with the algo type 4(EQ), stream 0(playback)\n");
    printf("  and save algo data into /tmp/awrpaf_test/test.pcm\n");
    printf("example(algo process):\n");
    printf("awrpaf_test -p 0 -F 2 -t 8 -f /tmp/awrpaf_test/algo_raw_data\n");
    printf("================================Usage================================\n");
    exit(-1);
}
