...
#define PIC_PATH "/tmp/mem_test/out.yuv"

dma_mem_des_t gMem;
char gPic[NV21_PIC_SIZE];

static int fillYuvData(int y, int uv)
{
    memset(&gPic[0], y, PIC_W * PIC_H);  //fill Y zoom
    memset(&gPic[PIC_W * PIC_H], uv, PIC_W * PIC_H / 2); //fill uv zoom
...
int main(int argc, char** argv)
{
    ...
    while (testcnt < 1) {
        ...
        printf("gMem.vir=%p gMem.phy=%p dmafd=%d,alloc len=%d\n",
               (void*)gMem.vir, (void*)gMem.phy, gMem.ion_buffer.fd_data.aw_fd, gMem.size);
