...
DvrMode *dvrs[MAX_TEST_CAMERA_NUM];

//static FILE *pfileH264 = NULL;
static int camera_nums = 0;
static int running_aging_flag = 1;
static int camera_id_table[MAX_TEST_CAMERA_NUM] = { 0,4,8,12};
static int  dvr_mode = 0;
static bool sub_stream_on = false;
...
void suspend_timer_cb(sigval_t sig)
{
    ...
    for (i = 0; i < camera_nums; i++) {
        ...
        dvrs[i] = new DvrNormalMode(camera_id_table[i]);
        tmp_dvr = dvrs[i];
    ...
    DisplayInit(DISP_HDMI_1280_720, dvrs[0]->getCameraId());
    //while(1)
    {
        for (i = 0; i < camera_nums; i++) {
#if ENABLE_WATER_MARK
            ...
            sprintf(bufname, "64,64,0,64,150,T527 SDK,64,250,ALLWINNER");
...
void argsAnalysis(int argc, char *argv[])
{
    ...
    for(int i = 1;i < argc;i++)
    {
        ...
        {// mdvr_modeode
            dvr_mode = atoi(argv[i+1]);
            if(dvr_mode == 360){
                camera_nums = 1;
                camera_id_table[0] = 360;
                ALOGD("Enter 360 mode!!!");
            }else{
                goto help;
            }
            break;
        ...
        {// time
            suspend_delay_second = atoi(argv[i+1]);
            ...
            }else if(suspend_delay_second == 0){
                running_aging_flag = 1;
                ALOGD("dvr_test set to always running");
            }
        }
        ...
        {// sub stream
            camera_nums = atoi(argv[i+1]);
            for(int n = 1;n <= camera_nums;n++)
            {
                camera_id_table[n - 1] = atoi(argv[i+1+n]);
            }
        }
    ...
    if(camera_nums <=0 || camera_nums > MAX_TEST_CAMERA_NUM){
        goto help;
    }
    return;
help:
           printf("---------------------------dvr_test---------------------------\n"
                  ...
                  "for example:\n");
           printf("/dvr_test -c 1 0                  //for test /dev/video0 \n");
           printf("/dvr_test -c 1 0 -t               //for test /dev/video0 and running always \n");
           printf("/dvr_test -c 1 0 -t 99            //suspend after 99 seconds \n");
           printf("/dvr_test -c 1 0 -s               //open sub stream for video0 \n");
           printf("/dvr_test -c 2 0 4 -s             //open sub stream for video0 video4 \n");
           printf("/dvr_test -c 4 0 4 8 12 -s        //open sub stream for video0 video4 video8 video12 only support 720p \n");
           printf("/dvr_test -m 360                  //open 360 by 0 4 8 12 camera compose,only support 720p\n");
           printf("---------------------------dvr_test---------------------------\n");
           exit(-1);
}
...
int main(int argc, char *argv[])
{
    ...
    sdk_log_setlevel(2);
    ALOGD("dvr_test ver:%s", MODULE_VERSION);
    ...
    for (i = 0; i < camera_nums; i++) {
        ...
        {
            case 360:
                dvrs[i] = new Dvr360Mode(camera_id_table[i]);  
            break;
            default:
                if(sub_stream_on){
                   dvrs[i] = new DvrSubStreamMode(camera_id_table[i]);
                }else{
                   dvrs[i] = new DvrNormalMode(camera_id_table[i]);
                }
    ...
    struct view_info vv[4][4] = {
                                {{ 0, 0, 1280, 800,0,0,0},{},{},{}},
                                {{ 0, 0, 640, 800,0,0,0},{ 640, 0, 640, 800,0,0,1},{},{}},
                                {{ 0, 0, 640, 400,0,0,0 },{ 640, 0, 640, 400,0,0,0 },{ 0, 400, 640, 400,0,0,0 },{}},
                                {{ 0, 0, 640, 400,0,0,0 },{ 640, 0, 640, 400,0,0,1 },{ 0, 400, 640, 400,0,0,2 },{ 640, 400, 640, 400,0,0,3 }},
                               };
    ...
    for (i = 0; i < camera_nums; i++) {
        ...
        dvrs[i]->setWaterMarkMultiple(bufname);
#endif
        ret = dvrs[i]->startPriview(vv[camera_nums - 1][i]);
        if (ret != 0) {
