...
#define INPUT_YUV "/tmp/encoder_test/video_800x480.yuv"
#define OUTPUT_FILE "/tmp/encoder_test/output/video_800x480"
...
static void PrintDemoUsage(void)
{
    ...
    printf("====================================== Usage ======================================\n");
    while (i < num) {
        printf("encoder_test %s %-32s %s", ArguTypeMapping[i].option, ArguTypeMapping[i].type,
...
static void ParseArguType(EncodeParam* encodeParam, TestParm* testParam, char* argument, char* value)
{
    ...
    switch (arg) {
        ...
        case SRC_W:
            ...
            ALOGD(" get srcW: %dp ", encodeParam->srcW);
            break;
        case SRC_H:
            sscanf(value, "%32u", &encodeParam->srcH);
            ALOGD(" get srcH: %dp ", encodeParam->srcH);

            break;
        case BIT_RATE:
            sscanf(value, "%32d", &encodeParam->bitRate);
            ALOGD(" bit rate: %d ", encodeParam->bitRate);
            break;
        case TEST_WAY:
...
{
    ALOGD("\t -s:%dx%d", encodeParam.srcW, encodeParam.srcH);

    ALOGD("\t -n:%d", encodeParam.frameCount);
    ALOGD("\t -f:%d", encodeParam.codecType);
    ALOGD("\t -p:%d", encodeParam.pixelFormat);
    ALOGD("\t -r:%d", encodeParam.frameRate);
    ALOGD("\t -d:%dx%d", encodeParam.dstW, encodeParam.dstH);
}
...
public:
    ...
    {
        //ALOGD(" encdoe data id=0x%x,pts=0x%llx",packet->id, packet->pts);
        if ((CODEC_JPEG == mParam->codecType) &&
            (ONLY_ONE_FRAME == mParam->frameCount)) {
            //Save all jpg files.
            char outPutfile[60];
            sprintf(outPutfile, "%s_%d.%s", testParam.outputFile, mJpgIndex++, "jpg");

            mFile = fopen(outPutfile, "wb");
            if (mFile == NULL) {
                ALOGE("open outFile fail");
                fclose(mFile);
                return -1;
            }
...
int encodeFile(EncodeParam* param, FILE* inFile, FILE* outFile)
{
    ...
    ALOGD("malloc pIonMem->vir=%p!", pIonMem->vir);
    ...
    ALOGD(" frameBitSize:%d", oneFrameLen);
    ALOGD(" totalFrameNum:%d %s", totalFrameNum, __TIME__);
    ...
    while (frameIndex < totalFrameNum) {
        ...
        packet.dataLen1 =  param->srcW * param->srcH / 2;
#endif
        ALOGD(" encdoe process1:[%d/%d] id=0x%x,pts=0x%llx", frameIndex, totalFrameNum, packet.id, packet.pts);
...
int encodeFile(EncodeParam* param, FILE* inFile, FILE* outFile)
{
    ...
    totalFrameNum = inputFileLen / oneFrameLen;
    ALOGD(" frameBitSize:%d", oneFrameLen);
    ALOGD(" totalFrameNum:%d", totalFrameNum);
#ifdef USE_PHY_ADDR
    ret = allocOpen(MEM_TYPE_CDX_NEW, pIonMem, NULL);
    if (ret < 0) {
        printf("ion_alloc_open failed");
    ...
    while (frameIndex < totalFrameNum) {
        ...
        ALOGD(" file %#X:[%d/%d]", outFile, frameIndex, totalFrameNum);
        frameIndex++;
    }
...
int singleEncoderTest()
{
    ...
    printfArgs(encodeParam);
    printf("output file is saved:%s", testParam.outputFile);
...
int _18_ch_EncoderTest()
{
    ...
    for (int i = 0; i < TOTAL_CHANNELS_NUM; i++) {
        ...
        if ((threadParm[i].inFile == NULL)) {
            ALOGE("open file fail:%s", testParam.intputFile);
        ...
        if ((threadParm[i].outFile == NULL)) {
            ALOGE("open file fail:%s.", outPutfile);
...
{
    ALOGD("encoderTest version:%s", MODULE_VERSION);
    ...
    testParam.testWay = 0;
    strcpy((char*)testParam.intputFile,INPUT_YUV);
    strcpy((char*)testParam.outputFile,OUTPUT_FILE);
