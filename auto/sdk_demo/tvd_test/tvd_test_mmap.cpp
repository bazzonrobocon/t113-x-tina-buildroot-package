...
enum v4l2_memory memtype = V4L2_MEMORY_MMAP;//=V4L2_MEMORY_MMAP V4L2_MEMORY_USERPTR V4L2_MEMORY_DMABUF
dma_mem_des_t gMem;

#include "hwdisp2.h"
#define PIC_W  720      // camera size
#define PIC_H  480
#define SCREEN_W 1024   // screen size
#define SCREEN_H 600
...
static void disp_show(unsigned long disp_buf, unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
    int screen_id = 0;
    int ch_id = 0;
    int lyr_id = 0;
    ...
    gdisp->aut_hwd_layer_set_src(layer0, &src, disp_buf);
...
#define DEFAULT_FILE_PATH "/tmp/tvd_test/"

void printUsage()
{
    printf("==========================Usage==========================\n");
    printf("tvd_test_mmap <videoX> <sel> <width> <height> <path> <format_mode> <test_cnt> <fps>\n");
...
int buffer_export(int v4lfd, enum v4l2_buf_type type, int index, int *dmafd)
{
    ...
    if (-1 == ioctl(v4lfd, VIDIOC_EXPBUF, &expbuf)) {
...
static int read_frame(int mode)
{
    ...
    if (save_flag == 0) {
        ...
        if (count <= 1)
            count = read_num;
        // buffer_export(fd, V4L2_BUF_TYPE_VIDEO_CAPTURE, buf.index, &dmafd);
        // printf("dmafd is %d !\r\n", dmafd);
        // sleep(1);
#if display_frame
        // printf("dbg1 gCameraDmafd[%d]=%d,buffers[%d].start[0]=0x%x\n",
        //      buf.index, gCameraDmafd[buf.index],
        //      buf.index, buffers[buf.index].start[0]);
        disp_show((unsigned long)buffers[buf.index].start[0], 0, 0, SCREEN_W, SCREEN_H);
#endif
    } else {
        count = 0;
    }
...
static int disp_init(int width, int height, unsigned int pixformat)
{
    ...
    /*DISP_FORMAT_YUV420_SP_UVUV ---- V4L2_PIX_FMT_NV12*/
        ...
        dev.layer_info.layer_config.info.fb.format = DISP_FORMAT_YUV420_SP_VUVU; //DISP_FORMAT_YUV420_SP_VUVU;
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 5) {
        ...
        width = atoi(argv[3]);
        height = atoi(argv[4]);
        sprintf(path_name, DEFAULT_FILE_PATH);
    } else if (argc == 6) {
        dev_id = atoi(argv[1]);
        sprintf(dev_name, "/dev/video%d", dev_id);
        sel = atoi(argv[2]);
...
#if display_frame
    releaseDE(0, 0, SCREEN_W, SCREEN_H);
#endif
