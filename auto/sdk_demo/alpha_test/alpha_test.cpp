...
#define PIC_W 800
#define PIC_H 480
#define INPUT_BMP_PATH "/tmp/alpha_test/pic_480p"
...
static int fillYuvData(void* ptr, int size)
{
    ...
    if (NULL == inputfd) {
        ALOGD("fopen %s failed", yuvfile);
    } else {
        ALOGD("fopen %s OK", yuvfile);
    }
