...
static int choose_sharespace(int fd, struct dsp_sharespace_t *msg, uint32_t choose)
{
    ...
    // ALOGD("%x, %x, %x",msg->dsp_write_addr,msg->arm_write_addr,msg->dsp_log_addr);
    switch (choose) {
...
static int refresh_dsp_log_space(int fd, struct dsp_sharespace_t *msg)
{
    ...
    // ALOGD("log_head_addr %x", msg->value.log_head_addr);
    // ALOGD("old_log_head, new_log_head = %x , %x", old_log_head, new_log_head);

    log_start_addr = 0;
    log_end_addr = log_start_addr + msg->dsp_log_size;
    // ALOGD("log_start_addr, log_end_addr = [%x, %x]", log_start_addr, log_end_addr);
    // ALOGD("log_head_addr = %x, arm_read_dsp_log_addr = %x", msg->value.log_head_addr, msg->arm_read_dsp_log_addr);
...
int main(int argc, char *argv[])
{
    ...
    if (fd < 0) {
        ALOGD("open device is error, fd = %d", fd);
        return -1;
    }
