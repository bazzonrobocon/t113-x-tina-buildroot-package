...
void getCallingProcessName(char *name) {
    ...
    if (fp > 0) {
        ...
        ::close(fp);
        fp = 0;
        CameraLogD("Calling process is: %s", name);
    } else {
        CameraLogE("Obtain calling process failed %s", proc_node);
    }
}
...
//    if (*end != delim) {
//        CameraLogE("Cannot find delimeter (%c) in str=%s", delim, str);
//        return -1;
//    }
//
...
CameraHardware::CameraHardware()
     ...
     */
    CameraLogV("CameraHardware2 construct");
    ...
    if (mV4L2CameraDevice == NULL) {
        CameraLogE("Failed to create V4L2Camera instance");
        return;
    }
...
CameraHardware::CameraHardware(int width,int height)
    : mPreviewWindow(),
      mCallbackNotifier(),
      mCameraManager(NULL),
      mCaptureWidth(width),
...
CameraHardware::~CameraHardware() {
    ...
    if (mCommandThread != NULL) {
        ...
        mCommandThread->join();
        CameraLogV("singal mCommandThread quit");
...
bool CameraHardware::commandThread() {
    ...
    if (mCommandThread->getThreadStatus() == THREAD_STATE_EXIT) {
        CameraLogD("commandThread exited");
        pthread_mutex_unlock(&mCommandMutex);
        return false;
    }
    ...
    switch (cmd) {
        case CMD_QUEUE_SET_COLOR_EFFECT: {
            ...
            CameraLogV("CMD_QUEUE_SET_COLOR_EFFECT: %d", new_image_effect);

            if (pV4L2Device->setImageEffect(new_image_effect) < 0) {
                CameraLogE("ERR(%s):Fail on mV4L2Camera->setImageEffect(effect(%d))", __FUNCTION__,
                           new_image_effect);
            }
            break;
        }
        case CMD_QUEUE_SET_WHITE_BALANCE: {
            ...
            CameraLogV("CMD_QUEUE_SET_WHITE_BALANCE: %d", new_white);

            if (pV4L2Device->setWhiteBalance(new_white) < 0) {
                CameraLogE("ERR(%s):Fail on mV4L2Camera->setWhiteBalance(white(%d))", __FUNCTION__,
                           new_white);
            }
            break;
        }
        case CMD_QUEUE_SET_EXPOSURE_COMPENSATION: {
            ...
            CameraLogV("CMD_QUEUE_SET_EXPOSURE_COMPENSATION: %d", new_exposure_compensation);

            if (pV4L2Device->setExposureCompensation(new_exposure_compensation) < 0) {
                CameraLogE("ERR(%s):Fail on mV4L2Camera->setBrightness(brightness(%d))",
        ...
        case CMD_QUEUE_SET_FLASH_MODE: {
            ...
            CameraLogV("CMD_QUEUE_SET_FLASH_MODE: %d", new_flash);

            if (pV4L2Device->setFlashMode(new_flash) < 0) {
                CameraLogE("ERR(%s):Fail on mV4L2Camera->setFlashMode(flash(%d))", __FUNCTION__,
                           new_flash);
            }
            break;
        }
        ...
        case CMD_QUEUE_START_ADAS_DETECTE: {
            CameraLogV("CMD_QUEUE_START_ADAS_DETECTE");
            mV4L2CameraDevice->adasInitialize();
            break;
        }
        case CMD_QUEUE_STOP_ADAS_DETECTE: {
            CameraLogV("CMD_QUEUE_STOP_ADAS_DETECTE");
        ...
        case CMD_QUEUE_START_WATER_MARK: {
            CameraLogV("CMD_QUEUE_START_WATER_MARK");
            mV4L2CameraDevice->waterMarkinit();
            break;
        }
        case CMD_QUEUE_STOP_WATER_MARK: {
            CameraLogV("CMD_QUEUE_STOP_WATER_MARK");
        ...
        case CMD_QUEUE_START_RENDER: {
            CameraLogV("CMD_QUEUE_START_RENDER");
            mPreviewWindow.startPreview();
            break;
        }
        case CMD_QUEUE_STOP_RENDER: {
            CameraLogV("CMD_QUEUE_STOP_RENDER");
        ...
        case CMD_QUEUE_TAKE_PICTURE: {
            CameraLogV("CMD_QUEUE_TAKE_PICTURE");
            break;
        }
        case CMD_QUEUE_PICTURE_MSG: {
            CameraLogV("CMD_QUEUE_PICTURE_MSG");
        ...
        default: {
            CameraLogW("unknown queue command: %d", cmd);
            break;
        }
    }
    return true;
...
void CameraHardware::initDefaultParameters() {
    ...
    CameraLogV("initDefaultParameters");
    ...
    CameraLogV("default preview size: %dx%d", mCaptureWidth, mCaptureHeight);
    ...
    mCallbackNotifier.setExifModel("T527");
    ...
    CameraLogV("[%s]:%s", CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES, value);
    ...
    CameraLogV("[%s]:%s", CameraParameters::KEY_SUPPORTED_PICTURE_SIZES, value);

    // frame rate
    p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES, "25");
    p.set(CameraParameters::KEY_PREVIEW_FRAME_RATE, "25");
    CameraLogV("[%s]:%s", CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES, "25");
    ...
    mFirstSetParameters = true;
    CameraLogV("CameraHardware::initDefaultParameters ok");
}
...
void CameraHardware::setCallbacks(notify_callback notify_cb, data_callback data_cb,
                                  data_callback_timestamp data_cb_timestamp, void *user) {
    CAMERA_LOG;
    mCallbackNotifier.setCallbacks((camera_notify_callback)notify_cb, (camera_data_callback)data_cb,
                                   (camera_data_timestamp_callback)data_cb_timestamp, (void *)user);
}
...
void CameraHardware::enableMsgType(int32_t msg_type) {
    if (msg_type & CAMERA_MSG_PREVIEW_FRAME) {
        ...
        // CameraLogV("vx=%d vy=%d sx=%d
        // sy%d",mcd->lcdxres,mcd->lcdyres,mCaptureWidth,mCaptureHeight);
...
void CameraHardware::stopPreview() {
    ...
    pthread_mutex_unlock(&mCommandMutex);
    pthread_cond_signal(&mCommandCond);
    CameraLogD("stopPreview send mCommandCond");
    Mutex::Autolock locker(&mObjectLock);
    doStopPreview();
}
...
status_t CameraHardware::startRecording() {
    CAMERA_LOG;
    CameraLogD("startRecording start");
    // video hint
    const char *valstr = mParameters.get(CameraParameters::KEY_RECORDING_HINT);
    if (valstr) {
        CameraLogV("KEY_RECORDING_HINT: %s -> true", valstr);
    }
...
void CameraHardware::stopRecording() {
    ...
    CameraLogD("F:%s,L:%d", __FUNCTION__, __LINE__);
    // mV4L2CameraDevice->setHwEncoder(false);
}
...
void CameraHardware::releaseRecordingFrame(const void *opaque) {
    if (mUseHwEncoder) {
        ...
        if (buffer_type != kMetadataBufferTypeCameraSource) {
            // CameraLogE("releaseRecordingFrame,error buffer type: %d", buffer_type);
        }

        if ((mCameraManager != NULL) && (mCameraManager->isOviewEnable())) {
        } else {
            const VencInputBuffer *inputBuffer = reinterpret_cast<const VencInputBuffer *>(
                reinterpret_cast<const char *>(opaque) + 4);
            mV4L2CameraDevice->releasePreviewFrame(inputBuffer->nID);
        }
    }
}

int CameraHardware::getTVINSystemType() { return mV4L2CameraDevice->getTVINSystemType(); }

int CameraHardware::getFrameWidth() { return mV4L2CameraDevice->getFrameWidth(); }

int CameraHardware::getFrameHeight() { return mV4L2CameraDevice->getFrameHeight(); }

void CameraHardware::releasePreviewFrame(int index,bool force) {
    mV4L2CameraDevice->releasePreviewFrame(index,force);
}
...
status_t CameraHardware::takePicture() {
    ...
    CameraLogD("video_hint is %d", video_hint);
    if (video_hint) {
...
void CameraHardware::setVideoCaptureSize(int video_w, int video_h) {
    CameraLogD("setVideoCaptureSize next version to do ......");
    ...
    if (ret < 0) {
        CameraLogE("setVideoCaptureSize tryFmtSize failed");
        return;
    }
...
void CameraHardware::getCurrentOriention(int *oriention) { *oriention = mOriention; }
...
status_t CameraHardware::setParameters(const char *p) {
    ...
    V4L2CameraDevice *pV4L2Device = mV4L2CameraDevice;
    if (pV4L2Device == NULL) {
        CameraLogE("%s : getCameraDevice failed", __FUNCTION__);
        return UNKNOWN_ERROR;
    }

    // picture mode
    const char *new_path = params.get(KEY_SNAP_PATH);
    CameraLogV("%s : snap new_path : %s", __FUNCTION__, new_path);
    if (new_path != NULL) {
    ...
    CameraLogV("%s : new_preview_format : %s", __FUNCTION__, new_preview_format);
    if (new_preview_format != NULL &&
        ...
        CameraLogE("%s : Only yuv420sp or yuv420p preview is supported", __FUNCTION__);
        return -EINVAL;
    }
    ...
    CameraLogV("%s : new_picture_width x new_picture_height = %dx%d", __FUNCTION__,
    ...
    if (0 < new_picture_width && 0 < new_picture_height) {
        ...
        CameraLogE("%s : error picture size", __FUNCTION__);
        return -EINVAL;
    }
    ...
    if (valstr) {
        CameraLogV("%s : KEY_RECORDING_HINT: %s", __FUNCTION__, valstr);
    ...
    CameraLogV("%s : new_jpeg_quality %d", __FUNCTION__, new_jpeg_quality);
...
char *CameraHardware::getParameters() {
    ...
    if (ret_str != NULL) {
        ...
        CameraLogE("%s: Unable to allocate string for %s", __FUNCTION__, params.string());
...
status_t CameraHardware::setCameraFlipStatus(int flipEnable) {
    ...
    CameraLogV("%s : flipEnable = %d", __FUNCTION__, flipEnable);
    return NO_ERROR;
}
// end
...
status_t CameraHardware::sendCommand(camera_cmd_t cmd, unsigned long arg1, int32_t arg2) {
    CameraLogV("%s: cmd = 0x%x, arg1 = %d, arg2 = %d", __FUNCTION__, cmd, arg1, arg2);

    /* TODO: Future enhancements. */
    switch (cmd) {
        case CAMERA_CMD_SET_WHITE_BALANCE:
            CameraLogD("sendCommand --- CAMERA_CMD_SET_WHITE_BALANCE");
            pthread_mutex_lock(&mCommandMutex);
        ...
        case CAMERA_CMD_SET_EXPOSURE_COMPENSATION:
            CameraLogD("sendCommand --- CAMERA_CMD_SET_EXPOSURE_COMPENSATION");
            pthread_mutex_lock(&mCommandMutex);
        ...
        case CAMERA_CMD_SET_COLOR_EFFECT:
            CameraLogD("sendCommand --- CAMERA_CMD_SET_COLOR_EFFECT");
            pthread_mutex_lock(&mCommandMutex);
        ...
        case CAMERA_CMD_START_FACE_DETECTION:
            CameraLogD("CMD_START_FACE_DETECTION not support!");
            return OK;
        case CAMERA_CMD_STOP_FACE_DETECTION:
            CameraLogD("CMD_STOP_FACE_DETECTION not support!");
            return OK;
#ifdef WATERMARK
        ...
        case CAMERA_CMD_STOP_RENDER:
            CameraLogV("CAMERA_CMD_STOP_RENDER");
            pthread_mutex_lock(&mCommandMutex);
        ...
        case CAMERA_CMD_START_RENDER:
            CameraLogV("CAMERA_CMD_START_RENDER");
            pthread_mutex_lock(&mCommandMutex);
        ...
        case CAMERA_CMD_PING:
            CameraLogD("CMD_PING not support!");
            return OK;
        case CAMERA_CMD_ENABLE_FOCUS_MOVE_MSG:
            CameraLogD("CMD_ENABLE_FOCUS_MOVE_MSG not support!");
            return OK;
        case CAMERA_CMD_SET_DISPLAY_ORIENTATION:
            CameraLogD("CMD_SET_DISPLAY_ORIENTATION not support!");
            return OK;
        default:
            break;
    }
...
status_t CameraHardware::dumpCamera(int fd) {
    CameraLogV("%s fd = %d", __FUNCTION__, fd);
...
status_t CameraHardware::doStartPreview() {
    ...
    if (camera_dev->isStarted() && mPreviewWindow.isPreviewEnabled()) {
        CameraLogD("CameraHardware::doStartPreview has been already started");
        return NO_ERROR;
    }
    ...
    CameraLogD("doStartPreview start");
    ...
    mVideoCaptureHeight = mCaptureHeight;
    // todo: no hardcode
    // uint32_t org_fmt = V4L2_PIX_FMT_YUYV;
    // uint32_t org_fmt = V4L2_PIX_FMT_NV21;
    int config_fmt = config_get_v4l2_fmt(mHardwareInfo.device_id);
    uint32_t org_fmt;
    switch (config_fmt) {
        case 0:  // nv21
        ...
        case 1:  // yuyv
            ...
            break;
        case 2:  // mjpeg
            org_fmt = V4L2_PIX_FMT_MJPEG;
            break;
        case 3:  // h264
            org_fmt = V4L2_PIX_FMT_H264;
            break;
        default:
            org_fmt = V4L2_PIX_FMT_NV21;
            break;
    }
    CameraLogD("CameraID[%d] mVideoCapture(%dx%d)", mHardwareInfo.device_id, mVideoCaptureWidth,
               mVideoCaptureHeight);
...
static void PrintParamDiff(const CameraParameters &current, const char *new_par) {
    ...
    while (next != NULL) {
        snprintf(tmp, sizeof(tmp), "%.*s", static_cast<int>(next - wrk), wrk);
            ...
            *val = '\0';
            ...
            if (in_current != NULL) {
                if (strcmp(in_current, val)) {
                    CameraLogD("=== Value changed: %s: %s -> %s", tmp, in_current, val);
                }
            } else {
                CameraLogD("+++ New parameter: %s=%s", tmp, val);
            }
        } else {
            CameraLogW("No value separator in %s", tmp);
        }
        wrk = next + 1;
        next = strchr(wrk, ';');
    }
