...
OMX_ERRORTYPE OSAL_QueueCreate(OSAL_QUEUE *queueHandle, int maxQueueElem) {
    ...
    if (!queue) return OMX_ErrorBadParameter;
    ...
    if (queue->first == NULL) return OMX_ErrorInsufficientResources;
...
int OSAL_QueueUrgent(OSAL_QUEUE *queueHandle, void *data, void *outdata) {
    ...
    if (queue->first->data == NULL) {
        ...
        *p = 0;
        OSAL_MutexUnlock(queue->qMutex);
        CameraLogD("OSAL_QueueUrgent CNT=%d qdata=%p data=%p \r\n", queue->numElem,
                   queue->first->data, data);
        return 0;
    } else {
        CameraLogD("xxOSAL_QueueUrgent CNT=%d qdata=%p data=%p outdata=%p \r\n", queue->numElem,
                   queue->first->data, data, outdata);
        *p = (int)(intptr_t)queue->first->data;
...
int OSAL_QueueSetElem(OSAL_QUEUE *queueHandle, void *data) {
    ...
    for (i = 0; i < queue->numElem; i++) {
        if ((queue->first->data == data) || (queue->last->data == data)) {
