...
DiHandle::~DiHandle() { DiHandleUnInit(); }
...
int DiHandle::DiHandleInit(int in_w, int in_h) {
    ...
    if ((mDiWidth != 0) && (mDiWidth != in_w)) {
        CameraLogE("mDiWidth should not be change twice\n");
        return -1;
    }
    CameraLogV("DiHandle::DiHandleInit w=[%d] h=[%d]", in_w, in_h);
    mDiWidth = in_w;
    mDiHeight = in_h;
    ...
    for (int i = 0; i < DI_HANDLE_BUFFER; i++) {
        ...
        if (ret < 0) {
            CameraLogD("DiBuffInit allocAlloc failed\n\n");
            return -1;
        }
        ...
        memset((void *)(mCamBuff[i].addrVirY + mDiWidth * mDiHeight), 0x80,
               mDiWidth * mDiHeight >> 1);
        // CameraLogV("------USE_DMABUF_API------dmafd----=%d\r\n",((ion_buffer_node*)mCamBuff[i].overlay_info)->fd_data.aw_fd);
        CameraLogV("DiHandleInit DiBuffInit---i=%d---vir=%ld\r\n", i, mCamBuff[i].addrVirY);
    }
    ...
    for (int i = 0; i < DI_HANDLE_BUFFER; i++) {
        ...
        memset((void *)(mdiBuffer[i].addrVirY + mDiWidth * mDiHeight), 0x80,
               mDiWidth * mDiHeight >> 1);
        CameraLogD("mdiBuffer[%d]------allocAlloc dmafd----=%d\r\n", i,
                   ((ion_buffer_node *)mdiBuffer[i].overlay_info)->fd_data.aw_fd);
    }
...
int DiHandle::DiHandleUnInit() {
    ...
    CameraLogV("DiHandle::DiHandleUnInit");
    paramStruct_t param_in;
    memset(&param_in, 0, sizeof(paramStruct_t));
    for (int i = 0; i < DI_HANDLE_BUFFER; i++) {
...
V4L2BUF_t *DiHandle::DiProcessLoop(V4L2BUF_t *camV4l2buf, int savePicBeforeDi, int enableTimeLog) {
    ...
    // CameraLogD("Dihandle: mDiReadId=%d vir=0x%x buf->width=%d,buf->height=%d\n",
    //         mDiReadId, mCamBuff[mDiReadId].addrVirY,camV4l2buf->width,camV4l2buf->height);

    memcpy((void *)mCamBuff[mDiReadId].addrVirY, (void *)camV4l2buf->addrVirY,
           camV4l2buf->width * camV4l2buf->height * 3 / 2);  // only cp data
    ...
    prebuf = (mDiReadId == 0) ? (&mCamBuff[DI_HANDLE_BUFFER - 1]) : (&mCamBuff[mDiReadId - 1]);
    ...
    if (mSkipCount <= 0) {
        memset(&param_in, 0, sizeof(paramStruct_t));
        param_in.ion_buffer.fd_data.aw_fd =
            ((ion_buffer_node *)mdiBuffer[mDiReadId].overlay_info)->fd_data.aw_fd;
        flushCache(MEM_TYPE_DMA, &param_in, NULL);
        ...
        mDiProcess->diProcess2((unsigned char *)preprebuf->addrPhyY,
                               (unsigned char *)prebuf->addrPhyY, (unsigned char *)buf->addrPhyY,
                               buf->width, buf->height, buf->format,
                               (unsigned char *)mdiBuffer[mDiReadId].addrPhyY, buf->width,
                               buf->height, buf->format, 0);

        mDiProcess->diProcess2((unsigned char *)preprebuf->addrPhyY,
                               (unsigned char *)prebuf->addrPhyY, (unsigned char *)buf->addrPhyY,
                               buf->width, buf->height, buf->format,
                               (unsigned char *)mdiBuffer[mDiReadId].addrPhyY, buf->width,
                               buf->height, buf->format, 1);
#else
    ...
    if (enableTimeLog) {
        ...
        time3 = time2 - time1;
        if (s_printcnt++ > 50) {
            CameraLogD("Dihandle use %lldus(%lldms)\n", time3, time3 / 1000);
            s_printcnt = 0;
        }
        time1 = time2;
    }
