...
void CameraParameters::set(const char *key, const char *value) {
    ...
    if (strchr(key, '=') || strchr(key, ';')) {
        // XXX CameraLogE("Key \"%s\"contains invalid character (= or ;)", key);
        return;
    }

    if (strchr(value, '=') || strchr(value, ';')) {
        // XXX CameraLogE("Value \"%s\"contains invalid character (= or ;)", value);
        return;
    }
...
void CameraParameters::remove(const char *key) { mMap.removeItem(String8(key)); }
...
static void parseSizesList(const char *sizesStr, Vector<Size> &sizes) {
    ...
    while (true) {
        ...
        if (success == -1 || (*sizeStartPtr != ',' && *sizeStartPtr != '\0')) {
            CameraLogE("Picture sizes string \"%s\" contains invalid character.", sizesStr);
            return;
        }
...
void CameraParameters::setPreviewFrameRate(int fps) { set(KEY_PREVIEW_FRAME_RATE, fps); }
...
void CameraParameters::setPreviewFormat(const char *format) { set(KEY_PREVIEW_FORMAT, format); }

const char *CameraParameters::getPreviewFormat() const { return get(KEY_PREVIEW_FORMAT); }
...
void CameraParameters::setPictureFormat(const char *format) { set(KEY_PICTURE_FORMAT, format); }

const char *CameraParameters::getPictureFormat() const { return get(KEY_PICTURE_FORMAT); }

void CameraParameters::dump() const {
    CameraLogD("dump: mMap.size = %d", mMap.size());
