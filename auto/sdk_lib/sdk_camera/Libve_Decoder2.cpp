...
static int GetStreamData(const void *in, char *buf0, int buf_size0, char *buf1, int buf_size1,
                         VideoStreamDataInfo *dataInfo) {
    CameraLogD("Starting get stream data!!");
    if (dataInfo->nLength <= buf_size0) {
        CameraLogV("The stream lengh is %d, the buf_size0 is %d", dataInfo->nLength, buf_size0);
        memcpy(buf0, in, dataInfo->nLength);
    } else {
        if (dataInfo->nLength <= (buf_size0 + buf_size1)) {
            CameraLogV("The stream lengh is %d, the buf_size0 is %d,the buf_size1 is %d",
                       dataInfo->nLength, buf_size0, buf_size1);
            memcpy(buf0, in, buf_size0);
            memcpy(buf1, static_cast<const char *>(in) + buf_size0,
...
void Libve_dec2(VideoDecoder **mVideoDecoder, const void *in, void *out,
    ...
    if (*mVideoDecoder == NULL) {
        CameraLogE("mVideoDecoder = NULL, return");
        return;
    }

    ret = RequestVideoStreamBuffer(*mVideoDecoder, dataInfo->nLength, &pBuf0, &size0, &pBuf1,
                                   &size1, 0);

    if (ret < 0) {
        CameraLogE("FUNC:%s, LINE:%d, RequestVideoStreamBuffer fail!", __FUNCTION__, __LINE__);
        return;
    }
    ...
    //* decode stream.
    ret = DecodeVideoStream(*mVideoDecoder, 0 /*eos */, 0 /*key frame only */, 0 /*drop b frame */,
                            0 /*current time */);
    ...
    if (ret == VDECODE_RESULT_FRAME_DECODED || ret == VDECODE_RESULT_KEYFRAME_DECODED) {
        ...
        CameraLogE("DecodeVideoStream fail,ret=%d", ret);
    }
}

int Libve_init2(VideoDecoder **mVideoDecoder, VideoStreamInfo *pVideoInfo, VConfig *pVconfig) {
    if (*mVideoDecoder != NULL) {
        CameraLogE("FUNC: %s fail, LINE: %d, mVideoDecoder is not NULL, please check it!",
    ...
    *mVideoDecoder = CreateVideoDecoder();
    ...
    if (InitializeVideoDecoder(*mVideoDecoder, pVideoInfo, pVconfig) != 0) {
        CameraLogE("initialize video decoder fail.");
...
int Libve_exit2(VideoDecoder **mVideoDecoder) {
    if (*mVideoDecoder == NULL) {
        CameraLogE("FUNC: %s, LINE: %d, mVideoDecoder == NULL", __FUNCTION__, __LINE__);
        return -1;
    }
