...
void yuv420sp_blending(int bg_width, int bg_height, int left, unsigned int top, int fg_width,
                       int fg_height, unsigned char *bg_y, unsigned char *bg_c, unsigned char *fg_y,
...
int region_bright_or_dark(int bg_width, int bg_height, int left, int top, int fg_width,
                          int fg_height, unsigned char *bg_y) {
    UNUSED(bg_height);
    unsigned char *bg_y_p = NULL;
...
void yuv420sp_blending_adjust_brightness(int bg_width, int bg_height, int left, int top,
                                         int fg_width, int fg_height, unsigned char *bg_y,
                                         unsigned char *bg_c, unsigned char *fg_y,
                                         unsigned char *fg_c, unsigned char *alph,
                                         int is_brightness) {
    UNUSED(bg_height);
    unsigned char *bg_y_p = NULL;
    ...
    if (bg_y == NULL || bg_c == NULL || fg_y == NULL || fg_c == NULL || alph == NULL) {
        CameraLogE("<yuv420sp_blending_adjust_brightness>NULL pointer error!");
        return;
    }
    bg_y_p = bg_y + top * bg_width + left;
    bg_c_p = bg_c + (top >> 1) * bg_width + left;
...
int watermark_blending(BackGroudLayerInfo *bg_info, WaterMarkInfo *wm_info,
    ...
    if (total_width > bg_info->width) {
        CameraLogE("<watermark_blending> error region(total_width=%d, bg_width=%d)", total_width,
                   bg_info->width);
        return -1;
    }
...
int watermark_blending_ajust_brightness(BackGroudLayerInfo *bg_info, WaterMarkInfo *wm_info,
    ...
    if (total_width > bg_info->width) {
        CameraLogE(
            "<watermark_blending_ajust_brightness> error region(total_width=%d, bg_width=%d)",
            total_width, bg_info->width);
        return -1;
    }
    ...
    if (ajust_Param->cur_frame == 0) {
        for (i = 0; i < wm_Param->number; i++) {
            id = wm_Param->id_list[i];
            is_brightness = region_bright_or_dark(bg_info->width, bg_info->height, pos_x,
                                                  wm_Param->pos.y, wm_info->single_pic[id].width,
                                                  wm_info->single_pic[id].height, bg_info->y);
            ajust_Param->Brightness[i] = is_brightness;

            yuv420sp_blending_adjust_brightness(
                bg_info->width, bg_info->height, pos_x, wm_Param->pos.y,
                wm_info->single_pic[id].width, wm_info->single_pic[id].height, bg_info->y,
                bg_info->c, wm_info->single_pic[id].y, wm_info->single_pic[id].c,
                wm_info->single_pic[id].alph, is_brightness);
            pos_x += wm_info->single_pic[id].width;
        }
    } else {
        for (i = 0; i < wm_Param->number; i++) {
