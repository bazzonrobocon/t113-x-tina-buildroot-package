...
static void argb2yuv420sp(unsigned char *src_p, unsigned char *alph, int width, int height,
...
static int getWordIndex(char *data, int len) {
    if (len == 1) {
        ...
        if (!memcmp(data, "日", 3))
            return 10;
        else if (!memcmp(data, "一", 3))
            return 11;
        else if (!memcmp(data, "二", 3))
            return 12;
        else if (!memcmp(data, "三", 3))
            return 13;
        else if (!memcmp(data, "四", 3))
            return 14;
        else if (!memcmp(data, "五", 3))
            return 15;
        else if (!memcmp(data, "六", 3))
            return 16;
        else if (!memcmp(data, "星", 3))
            return 17;
        else if (!memcmp(data, "期", 3))
            return 18;
    } else {
        return 19;
    }
...
void *initialWaterMark(int wm_height) {
    ...
    CameraLogD("water mark initialize, height:%d", wm_height);
    wm_ctrl = (WATERMARK_CTRL *)malloc(sizeof(WATERMARK_CTRL));
    if (wm_ctrl == NULL) {
        CameraLogE("Alloc wm_ctrl error(%s)", strerror(errno));
        return NULL;
    }
    ...
    for (i = 0; i < watermark_pic_num; ++i) {
        ...
        fp = fopen(filename, "r");
        if (NULL == fp) {
            CameraLogE("Fail to open file %s(%s)!", filename, strerror(errno));
            goto OPEN_FILE_ERR;
        }
        ...
        wm_ctrl->wminfo.single_pic[i].y = (unsigned char *)malloc(
            wm_ctrl->wminfo.single_pic[i].width * wm_ctrl->wminfo.single_pic[i].height * 5 / 2);
        ...
        fread(buf, wm_ctrl->wminfo.single_pic[i].width * wm_ctrl->wminfo.single_pic[i].height * 4,
              1, fp);

        argb2yuv420sp(buf, wm_ctrl->wminfo.single_pic[i].alph, wm_ctrl->wminfo.single_pic[i].width,
                      wm_ctrl->wminfo.single_pic[i].height, wm_ctrl->wminfo.single_pic[i].y,
                      wm_ctrl->wminfo.single_pic[i].c);
...
void *initWaterMarkMultiple() {
    ...
    if (NULL == mWaterMarkMultiple) {
        CameraLogE("Fail to initialize mWaterMarkMultiple!");
        return NULL;
    }
...
int releaseWaterMarkMultiple(void *multi) {
    ...
    CameraLogI("water mark multiple release");
...
int doWaterMark(WaterMarkInData *data, void *ctrl) {
    ...
    if (ctrl == NULL || data == NULL) {
        CameraLogE("Input parameters error!");
        return -1;
    }
    if (data->display == NULL || (buflen = strlen(data->display)) == 0) {
        CameraLogE("invalid water mark display buffer!");
        return -1;
    }
...
int doWaterMarkMultiple(WaterMarkInData *data, void *ctrl, void *multi, char *content,
                        char *time_watermark) {
    CAMERA_LOG;
    const char *delims = ",";
