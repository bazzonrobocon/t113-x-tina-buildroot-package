...
#define CAMHAL_GRALLOC_USAGE GRALLOC_USAGE_SW_READ_RARELY | GRALLOC_USAGE_SW_WRITE_OFTEN
...
void PreviewWindow::PreviewTvOut(HwDisplay *mcd, int screen_id, unsigned int *hlay,
    ...
    if ((NULL == mcd) || (NULL == hlay)) {
        CameraLogD("%s input arg error\r\n", __FUNCTION__);
    }

    CameraLogD("test_layer%d_tvout_ntsc begin\r\n", screen_id);
...
PreviewWindow::PreviewWindow()
    ...
    memset(&mvi, 0, sizeof(view_info));

    int ret = allocOpen(mMemType, &mPreviewMemops, NULL);
    if (ret < 0) {
        CameraLogE("ion_alloc_open failed");
    }
...
PreviewWindow::~PreviewWindow() {
    ...
    if ((mPreviewMemops.ops != NULL) || (mMemType == mPreviewMemops.memType)) {
        ...
        if (ret < 0) {
            CameraLogE("~PreviewWindow:allocClose MEM_TYPE_CDX_NEW failed");
        }
    }
...
status_t PreviewWindow::setPreviewDisplayOn() {
    CAMERA_LOG
    CameraLogD("setPreviewDisplayOn");
    status_t res = NO_ERROR;
    Mutex::Autolock locker(&mObjectLock);
    if(mvi.w <= 0||mvi.h <=0 || mvi.w >=4000 || mvi.h > 8000){
        CameraLogD("setPreviewDisplayOn but config not display!");
        return 0;
    }
    ...
    for (i = 0; i < DISP_NB_BUFFER; i++) {
        if (mDispBuff[i].addrVirY == 0) {
            ...
            CameraLogD("setPreviewDisplayOn mDispBuff[%d] dmafd:%d vir:0x%lx", i,
    ...
    mHwlayerScreen0 = mcd->aut_hwd_layer_request(&mvi, 0, mvi.channel, mvi.layer);
    ...
    if (mtvout) {
        ...
        if (mtvout == DISP_TV_MOD_NTSC) {
            SurfaceTV.w = 720;
            SurfaceTV.h = 480;
        } else if (mtvout == DISP_TV_MOD_PAL) {
            SurfaceTV.w = 720;
            SurfaceTV.h = 576;
        }else{
            SurfaceTV.w = 1280;
        ...
        //----------------------2:ui TV OUT CFG-------------------------------
        // this have a bug,because linux fb have pan display,you must have a timer (10ms eg) in your
        // app to check the base layer crop addr or modify the kernel driver,dor disp pan to swap
        // the base adrr for fb0 layer
        mHwlayerScreen1_ui = mcd->aut_hwd_layer_request(&SurfaceTV, 1, 1, 0);

        disp_layer_config2 *fb0Config = mcd->aut_hwd_get_layer_config_by_id(0, 1, 0);
...
status_t PreviewWindow::setPreviewDisplayOff() {
    CAMERA_LOG;
    CameraLogD("setPreviewDisplayOff");
    status_t res = NO_ERROR;
    Mutex::Autolock locker(&mObjectLock);
    isPreviewDispEn = false;
    ...
    for (i = 0; i < DISP_NB_BUFFER; i++) {
        if (mDispBuff[i].addrVirY != 0) {
            mPreviewMemops.vir = mDispBuff[i].addrVirY;
            allocFree(mMemType, &mPreviewMemops, NULL);
            // CameraLogD("setPreviewDisplayOff mDispBuff[%d]=%d vir=0x%lx",
...
status_t PreviewWindow::setPreviewParam(struct view_info pvi, struct src_info psi, int tvout,
                                        int tvch) {
    CameraLogD("setPreviewParam view_info(%d,%d,%d,%d), src_info(%d,%d,%d,), tvout:%d, tvch:%d",
               pvi.x, pvi.y, pvi.w, pvi.h, psi.w, psi.h, psi.format, tvout, tvch);
    mvi = pvi;
    msi = psi;
    mtvout = tvout;
    mtvch = tvch;
...
bool PreviewWindow::onNextFrameAvailable(const void *frame, int flipEnable, bool isOview) {
    ...
    UNUSED(isOview);
    Mutex::Autolock locker(&mObjectLock);
    if (!isPreviewDispEn) return true;
    ...
    //     CameraLogD("preview start count down:%d", previewSkipCounter);
    //     return 0;
    // }
    ...
    if ((pv4l2_buf->width <= 720) && (mDi)) {
        ...
        if (NULL == pDiBuff) {
            CameraLogD("Dihandle failed,use this origin data cam [%d]w[%d] h[%d]\n",
                       pv4l2_buf->thumbFormat, pv4l2_buf->width, pv4l2_buf->height);
        } else {
            CameraLogD("Dihandle success\n");
            pv4l2_buf = pDiBuff;
        }
    }
    ...
    // struct view_info crop = {pv4l2_buf->crop_rect.left, pv4l2_buf->crop_rect.top,
    //                          pv4l2_buf->crop_rect.width, pv4l2_buf->crop_rect.height};
    ...
    CameraLogV("onNextFrameAvailable mDispBuff[%d]=%d", cur_idx, mDispBuff[cur_idx].dmafd);
    int dmode = 0;  // aut_get_disp_rotate_mode();
    if (dmode > 0) {
        ...
        switch (dmode) {
            ...
            case TR_ROT_90:  // note:dist w*h become h*w
                g2dRotate(imgtr.trfd, G2D_ROTATE90, (unsigned char *)pv4l2_buf->addrPhyY, src.w,
                          src.h, (unsigned char *)mDispBuff[cur_idx].addrPhyY, src.h, src.w);
                ww = src.h;
                hh = src.w;
                break;
            case TR_ROT_180:
                g2dRotate(imgtr.trfd, G2D_ROTATE180, (unsigned char *)pv4l2_buf->addrPhyY, src.w,
                          src.h, (unsigned char *)mDispBuff[cur_idx].addrPhyY, src.w, src.h);
                ww = src.w;
                hh = src.h;
                break;
            case TR_ROT_270:  // note:dist w*h become h*w
                g2dRotate(imgtr.trfd, G2D_ROTATE270, (unsigned char *)pv4l2_buf->addrPhyY, src.w,
                          src.h, (unsigned char *)mDispBuff[cur_idx].addrPhyY, src.h, src.w);
                ww = src.h;
                hh = src.w;
                break;
            case TR_HFLIP:
                g2dRotate(imgtr.trfd, G2D_FLIP_VERTICAL, (unsigned char *)pv4l2_buf->addrPhyY,
                          src.w, src.h, (unsigned char *)mDispBuff[cur_idx].addrPhyY, src.w, src.h);
                ww = src.w;
                hh = src.h;
                break;
            case TR_HFLIP_ROT_90:  // note:dist w*h become h*w
                g2dRotate(imgtr.trfd, G2D_MIRROR45, (unsigned char *)pv4l2_buf->addrPhyY, src.w,
                          src.h, (unsigned char *)mDispBuff[cur_idx].addrPhyY, src.h, src.w);
                ww = src.h;
                hh = src.w;
                break;
            case TR_VFLIP:
                g2dRotate(imgtr.trfd, G2D_FLIP_HORIZONTAL, (unsigned char *)pv4l2_buf->addrPhyY,
                          src.w, src.h, (unsigned char *)mDispBuff[cur_idx].addrPhyY, src.w, src.h);
                ww = src.w;
                hh = src.h;
                break;
            case TR_VFLIP_ROT_90:  // note:dist w*h become h*w
                g2dRotate(imgtr.trfd, G2D_MIRROR135, (unsigned char *)pv4l2_buf->addrPhyY, src.w,
                          src.h, (unsigned char *)mDispBuff[cur_idx].addrPhyY, src.h, src.w);
                ww = src.h;
                hh = src.w;
                break;
    ...
    if (mHwDisplay->CameraDispEnableFlag[0] == true) {
        ...
        if (ret < 0) {
            CameraLogE("onNextFrameAvailable set video0 src error");
        }
    ...
    if (mHwDisplay->CameraDispEnableFlag[1] == true) {
        ...
        if (ret < 0) {
            CameraLogE("onNextFrameAvailable set video1 src error");
        }
...
#endif
    {
        ...
        if (mDispBuff[mDispCnt].addrVirY != NULL) {
            ...
            // CameraLogV("------xx--mDispBuff dmafd----=%d\n", mDispBuff[mDispCnt].dmafd);
        }
        return true;
    }
