...
typedef struct {
    ...
    unsigned int
        bTopFieldFirst;  // video infomation <0-is not top_field_first; 1-is top_field_first>
} DiParaT;

DiProcess::DiProcess() : mDiFd(-1), mDiReqId(-1) {
...
DiProcess::~DiProcess() {
    ...
    release();
    CameraLogD("F:%s ok ,L:%d\n", __FUNCTION__, __LINE__);
}

int DiProcess::init() {
    if (mDiFd > 0) {
        CameraLogE("DiProcess is already init,F:%s,L:%d\n", __FUNCTION__, __LINE__);
        return -1;
    }
    mDiFd = open("/dev/deinterlace", O_RDWR, 0);
    if (mDiFd <= 0) {
        CameraLogE("open /dev/deinterlace failed,F:%s,L:%d\n", __FUNCTION__, __LINE__);
        return -1;
    }
    CameraLogD("open /dev/deinterlace OK, F:%s,L:%d\n", __FUNCTION__, __LINE__);

#if defined(_T507_) || defined(_T7_P1_)
    int ret = -1;
    ret = ioctl(mDiFd, DI_REQUEST, &mDiReqId);
    if (ret != 0) {
        CameraLogE("DI_REQUEST failed,F:%s,L:%d\n", __FUNCTION__, __LINE__);
        return -1;
    }
    CameraLogD("DI_REQUEST OK, F:%s,L:%d, id:%d\n", __FUNCTION__, __LINE__, mDiReqId);
#endif
    return NO_ERROR;
}

int DiProcess::diProcess2(unsigned char *pre_src, unsigned char *now_src, unsigned char *next_src,
                          int src_width, int src_height, int in_format, unsigned char *dst,
                          int dst_width, int dst_height, int out_format, int nField) {
    int ret = -1;
    __di_para_t2 di_para;

    if (mDiFd < 0) {
        CameraLogE("deinterlace is not init!F:%s,L:%d\n", __FUNCTION__, __LINE__);
        return -1;
    }
...
void DiProcess::diProcess1(unsigned char *presrc, int pre_width, int pre_height, unsigned char *src,
                           int src_width, int src_height, unsigned char *dst, int dst_width,
                           int dst_height, int nField) {
    int ret = 0;
    DiParaT mDiParaT;
    ...
    if (ret < 0) {
        CameraLogE("%s hw di ioctl, DI_IOCSTART failed\n", __FUNCTION__);
        return;
    }
}
...
int DiProcess::release() {
    if (mDiFd > 0) {
        if (mDiReqId >= 0) {
            ...
            if (ret < 0) {
                CameraLogD("DI_RELEASE failed id=%d\n", mDiReqId);
            }
            CameraLogD("DI_RELEASE, F:%s,L:%d, id:%d\n", __FUNCTION__, __LINE__, mDiReqId);
            mDiReqId = -1;
        }
        close(mDiFd);
        mDiFd = -1;
        CameraLogD("DiProcess,release ok, F:%s,L:%d\n", __FUNCTION__, __LINE__);
    }
    return NO_ERROR;
}
