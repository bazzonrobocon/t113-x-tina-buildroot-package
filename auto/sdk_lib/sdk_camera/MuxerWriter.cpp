...
int __CdxRead(CdxWriterT *writer, void *buf, int size) {
    ...
    if (muxer_writer->file_mode == FD_FILE_MODE) {
        ...
        CameraLogW("__CdxRead name %s fd=%d pbuf=%p len=%d", namer, muxer_writer->fd_file, pbuf,
                   size);
        if (ret < 0) {
            CameraLogE("CdxRead read failed\n");
            return -1;
        }
    } else if (muxer_writer->file_mode == FP_FILE_MODE) {
        ret = fread(pbuf, 1, size, muxer_writer->fp_file);
        if (ret < 0) {
            CameraLogE("CdxRead fread failed\n");
            return -1;
        }
    }
    return ret;
...
int __CdxWrite(CdxWriterT *writer, void *buf, int size) {
    ...
    if (muxer_writer->file_mode == FD_FILE_MODE) {
        ...
        // CameraLogW("write name %s fd=%d pbuf=%p len=%d",namer,muxer_writer->fd_file, pbuf, size);
        ...
        if (havewrites > falloc_size) {
            CameraLogE("__CdxWrite find error,fa size=%d,but write %d bytes", falloc_size,
                       havewrites);
            return 0;
        }
        ret = write(muxer_writer->fd_file, pbuf, size);
        if (ret < 0) {
            CameraLogE("CdxWrite write failed %s fd=%d name=%s\n", strerror(errno),
                       muxer_writer->fd_file, muxer_writer->file_path);
            return 0;
        }
        ...
        if (muxfdmgr_fds[camid].cnt % WRITE_CNT_FOR_SYNC == 0) {
            ...
            if (ret < 0) {
                CameraLogE("CdxWrite fsync failed %s fd=%d camid=%d\n", strerror(errno),
                           muxer_writer->fd_file, camid);
            }
            long long timeend = GetNowUs();

            CameraLogV("camid %d fsync time is %lld ", camid, timeend - timestart);
        }
    } else if (muxer_writer->file_mode == FP_FILE_MODE) {
        ret = fwrite(pbuf, 1, size, muxer_writer->fp_file);
        if (ret < 0) {
            CameraLogE("CdxWrite fwrite failed\n");
            return 0;
        }
    }
    return ret;
...
long __CdxSeek(CdxWriterT *writer, long moffset, int mwhere) {
    ...
    if (muxer_writer->file_mode == FD_FILE_MODE) {
        ...
        memset(namer, 0, 256);
        readlink(nn, namer, 256);
        CameraLogW("seek name %s fd=%d where =%d off =%ld", namer, muxer_writer->fd_file, mwhere,
                   moffset);
        ret = lseek(muxer_writer->fd_file, moffset, mwhere);
        if (ret < 0) {
            CameraLogE("CdxSeek lseek failed\n");
            return -1;
        }
    } else if (muxer_writer->file_mode == FP_FILE_MODE) {
        ret = fseek(muxer_writer->fp_file, moffset, mwhere);
        if (ret < 0) {
            CameraLogE("CdxSeek fseek failed\n");
            return -1;
        }
    }
    return ret;
...
long __CdxTell(CdxWriterT *writer) {
    ...
    if (muxer_writer->file_mode == FD_FILE_MODE) {
        ...
        CameraLogW("write tell %s fd=%d ", namer, muxer_writer->fd_file);
#ifdef CDX_V27
...
int xxCreateMyFile(char *szFileName, int nFileLength) {
    int res, fd;
    // CameraLogW("CreateMyFile %s",szFileName);
    ...
    // CameraLogD("===xxCreateMyFile,%s,camid=%d\n", szFileName, camid);
    muxfdmgr_fds[camid].fileFallocSize = nFileLength;
    // CameraLogW("fallocate start!len=%d\n",nFileLength);
    if ((res = fallocate(fd, FALLOC_FL_KEEP_SIZE, 0, nFileLength))) {
        fprintf(stderr, "fallocate line %d len=%d res = %d %s\n", __LINE__, nFileLength, res,
                strerror(errno));
        fsync(fd);
...
int MWOpen(CdxWriterT *writer) {
    ...
    if (muxer_writer->file_path == NULL) {
        CameraLogE("muxer_writer->file_path is NULL\n");
        return -1;
    }
    if (muxer_writer->file_mode == FD_FILE_MODE) {
        CameraLogW("MWOpen--%s-", muxer_writer->file_path);
#if 0
        int camid;
        ...
        if (muxer_writer->fd_file < 0) {
            CameraLogE("muxer_writer->fd_file open failed %s[%d]\n", muxer_writer->file_path,
                       errno);
            return -1;
        }
        muxfdmgr_fds[camid].fd = muxer_writer->fd_file;
        CameraLogW("MWOpen--%s-fd=%d----camid=%d", muxer_writer->file_path, muxer_writer->fd_file,
                   camid);
        ...
        if (muxer_writer->fp_file == NULL) {
            CameraLogE("muxer_writer->fp_file fopen failed\n");
            return -1;
        }
    }
    return 0;
...
int MWClose(CdxWriterT *writer) {
    ...
    int ret = 0;
    if (muxer_writer->file_mode == FD_FILE_MODE) {
        CameraLogW("close---fd=%d path=%s camfd0=%d camfd1=%d", muxer_writer->fd_file,
        ...
        memset(namer, 0, 256);
        readlink(nn, namer, 256);
        CameraLogW("close---%d %s", muxer_writer->fd_file, namer);
        // fsync(muxer_writer->fd_file);//too slow
        ret = close(muxer_writer->fd_file);
        CameraLogW("close---%d %s", muxer_writer->fd_file, namer, ret);
    ...
    if (ret < 0) {
        CameraLogE("__CdxClose() failed %s", strerror(errno));
    }
    return ret;
}
...
void CdxWriterDestroy(CdxWriterT *writer) {
    ...
    if (muxer_writer == NULL) {
        CameraLogW("writer is NULL, no need to be destroyed\n");
        return;
    }
