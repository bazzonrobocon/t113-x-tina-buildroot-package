...
static int cubic_coeff[] = {
    // 32-taps
    0,   256, 0,   0,   -3,  255, 4,   0,   -6,  254, 10,  0,   -9,  251, 16,  0,   -11, 247, 23,
    -1,  -13, 242, 31,  -2,  -15, 236, 39,  -3,  -16, 229, 48,  -4,  -17, 222, 58,  -5,  -18, 214,
    68,  -6,  -18, 205, 78,  -8,  -18, 196, 89,  -9,  -18, 186, 100, -10, -17, 176, 111, -12, -17,
    166, 122, -13, -16, 155, 133, -14, -15, 144, 144, -15, -14, 133, 155, -16, -13, 122, 166, -17,
    -12, 111, 176, -17, -10, 100, 186, -18, -9,  89,  196, -18, -8,  78,  205, -18, -6,  68,  214,
    -18, -5,  58,  222, -17, -4,  48,  229, -16, -3,  39,  236, -15, -2,  31,  242, -13, -1,  23,
    247, -11, 0,   16,  251, -9,  0,   10,  254, -6,  0,   4,   255, -3,
};
...
void do_scaler(ISP_CTX *ispctx, unsigned char *psrc, unsigned char *pdst, int src_w, int src_h,
    ...
    for (i = 0; i < ispctx->SC_dst_height; i++) {
        ...
        {
            if (base_line >= ispctx->height) base_line = ispctx->height - 1;

            base_linep1 = base_line + 1;
            if (base_linep1 >= ispctx->height) base_linep1 = ispctx->height - 1;
        }
    ...
    switch (fmt) {  // 0-yuv420, 1-yuv422, 2-yvu420, 3-yvu422,4-yuyv422, 5-32x32,8-argb8888,
                    // 9-rgba8888, 10-abgr8888, 11-bgra8888
        case 0: {
    ...
    for (i = 0; i < dst_height; i++) {
        ...
        {
            if (base_line >= src_height) base_line = src_height - 1;

            base_linep1 = base_line + 1;
            if (base_linep1 >= src_height) base_linep1 = src_height - 1;
        }
...
int scaler(unsigned char *psrc, unsigned char *pdst, int src_w, int src_h, int dst_w, int dst_h,
    ...
    if (fmt == 0) ispctx->outpicsize = dst_w * dst_h * 1.5;
