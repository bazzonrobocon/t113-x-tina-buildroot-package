...
CallbackNotifier::~CallbackNotifier() { CameraLogV("CallbackNotifier disconstruct"); }
...
void CallbackNotifier::setCallbacks(camera_notify_callback notify_cb, camera_data_callback data_cb,
    ...
    CameraLogV("%s: %p, %p, %p (%p)", __FUNCTION__, notify_cb, data_cb, data_cb_timestamp, user);
...
void CallbackNotifier::enableMessage(uint msg_type) {
    CameraLogV("%s: msg_type = 0x%x", __FUNCTION__, msg_type);

    Mutex::Autolock locker(&mObjectLock);
    mMessageEnabler |= msg_type;
    CameraLogV("**** Currently enabled messages:");
...
void CallbackNotifier::disableMessage(uint msg_type) {
    ...
    Mutex::Autolock locker(&mObjectLock);
    mMessageEnabler &= ~msg_type;
    CameraLogV("**** Currently disable messages:");
    PrintMessages(mMessageEnabler);
}
...
status_t CallbackNotifier::adasResultMsg(void *adas) {
    ...
    if (isMessageEnabled(CAMERA_MSG_ADAS_METADATA)) {
        ...
        if (NULL == cam_buff) {
            CameraLogE("%s: Memory failure in CAMERA_MSG_ADAS_METADATA", __FUNCTION__);
            return NO_MEMORY;
        }
        mAdasDataCB(CAMERA_MSG_ADAS_METADATA, (const char *)cam_buff, 0, adas, mCallbackCookie);
        if (!cam_buff) free(cam_buff);
    }
    return NO_ERROR;
}
#endif
...
void CallbackNotifier::onNextFrameHW(const void *frame, int channel) {
    ...
    if (isMessageEnabled(CAMERA_MSG_VIDEO_FRAME) && isVideoRecordingEnabled()) {
        mDataCBTimestamp(pbuf->timeStamp, CAMERA_MSG_VIDEO_FRAME, (const char *)pbuf,
                         mCallbackCookie);
    }
}
...
void CallbackNotifier::notifyPictureMsg(const void *frame) {
    ...
    if (isMessageEnabled(CAMERA_MSG_RAW_IMAGE)) {
        ...
        if (NULL == dummyRaw) {
            CameraLogE("%s: Memory failure in CAMERA_MSG_PREVIEW_FRAME", __FUNCTION__);
            return;
        }
        mDataCB(CAMERA_MSG_RAW_IMAGE, dummyRaw, NULL, mCallbackCookie);
        if (!dummyRaw) free(dummyRaw);
...
bool CallbackNotifier::takePicture(const void *frame, bool is_continuous) {
    ...
    uint8_t *src_addr_virY = 0;
    uint8_t *src_addr_virC = 0;
    int src_width = 0;
    int src_height = 0;
    RECT_t src_crop;
    if ((pbuf->isThumbAvailable == 1) && (pbuf->thumbUsedForPhoto == 1)) {
        ...
        src_addr_virY = pbuf->thumbAddrVirY;
        ...
        src_addr_virY = pbuf->addrVirY;
        src_addr_virC = pbuf->addrVirC;
    ...
    jpeg_enc.addrY = src_addr_virY;  // src_addr_phy;
    // jpeg_enc.addrC = src_addr_vir + ALIGN_16B(src_width) * src_height;
    // jpeg_enc.addrC = src_addr_vir + ALIGN_16B(src_width) * ALIGN_32B(src_height);
    jpeg_enc.addrC = src_addr_virC;
    // src_addr_phy + ALIGN_16B(src_width) * src_height;
    jpeg_enc.src_w = src_width;
    jpeg_enc.src_h = src_height;
    jpeg_enc.pic_w = mPictureWidth;
    jpeg_enc.pic_h = mPictureHeight;
    CameraLogD("Y %p C %p", jpeg_enc.addrY, jpeg_enc.addrC);
    if (src_format == V4L2_PIX_FMT_NV21 || src_format == V4L2_PIX_FMT_YUYV) {
    ...
    if ((src_crop.width != jpeg_enc.src_w) || (src_crop.height != jpeg_enc.src_h)) {
    ...
    CameraLogV("addrY: %p, addrC: %p pic: %dx%d, quality: %d, rotate: %d", jpeg_enc.addrY,
               jpeg_enc.addrC, jpeg_enc.pic_w, jpeg_enc.pic_h, jpeg_enc.quality, jpeg_enc.rotate);

    pOutBuf = (void *)malloc(mPictureWidth * mPictureHeight);
    int ret = JpegEnc(pOutBuf, &bufSize, &jpeg_enc);
    if (ret < 0) {
        CameraLogE("JpegEnc failed");
        return false;
    }
    // CameraLogV("hw enc time: %lld(ms), size: %d", (systemTime() - lasttime)/1000000, bufSize);
    ...
    if (NULL != jpeg_buff) {
        ...
        if (!jpeg_buff) free(jpeg_buff);
    } else {
        CameraLogE("%s: Memory failure in CAMERA_MSG_COMPRESSED_IMAGE", __FUNCTION__);
    }

    CameraLogV("taking photo end");
    return true;
}
...
void CallbackNotifier::setWaterMarkMultiple(char *mWaterMark) {
    CameraLogV("%s***********%s", __FUNCTION__, mWaterMark);
...
void CallbackNotifier::getCurrentDateTime() {
    ...
    time(&t);
    tm_t = localtime(&t);
    sprintf(mDateTime, "%4d:%02d:%02d %02d:%02d:%02d", tm_t->tm_year + 1900, tm_t->tm_mon + 1,
