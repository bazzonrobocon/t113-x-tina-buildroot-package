...
void DisplayShowPic(HwDisplay *mcd) {
    ...
    unsigned long disp_vaddr = (unsigned long)CdcMemPalloc(
        dispmemops, (DISP_PIC_W * DISP_PIC_H * 4 + 0xfff) & ~0xfff, NULL, NULL);
    unsigned long disp_paddr =
        (unsigned long)CdcMemGetPhysicAddressCpu(dispmemops, (void *)disp_vaddr);
    char filename[64];
    ...
    if (NULL == fpff) {
        ...
        if (NULL == fpff) {
            CameraLogE("fopen %s ERR ", filename);
        } else {
            CameraLogE("fopen %s OK ", filename);
            fread((void *)disp_vaddr, 1, DISP_PIC_W * DISP_PIC_H * 3 / 2, fpff);
            fclose(fpff);
        }
    } else {
        CameraLogD("fopen %s OK ", filename);
        fread((void *)disp_vaddr, 1, DISP_PIC_W * DISP_PIC_H * 3 / 2, fpff);
        fclose(fpff);
    }
    ...
    CameraLogD("layer_request  layer0 ");
    ...
    CameraLogD("aut_hwd_layer_render video_layer0");
    sleep(3000);
