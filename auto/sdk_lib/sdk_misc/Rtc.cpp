...
int setDateTime(struct tm* ptm)
{
    ...
    if (settimeofday(&tv, NULL) < 0) {
        ALOGE("Set system date and time error.");
        return -1;
    }
    ...
    ALOGD("  datetime->tm_year=%d", local->tm_year + 1900);
    ALOGD("  datetime->tm_mon=%d", local->tm_mon);
    ALOGD("  datetime->tm_mday=%d", local->tm_mday);
    ALOGD("  datetime->tm_hour=%d", local->tm_hour);
    ALOGD("  datetime->tm_min=%d", local->tm_min);
    ALOGD("  datetime->tm_sec=%d", local->tm_sec);
