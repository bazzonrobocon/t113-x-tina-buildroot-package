...
#define LOG_TAG "awrpaf_component"
#include <sdklog.h>
                                     ...
                                     int stream, int enable)
{
    ...
    if (fd < 0) {
        ALOGE("open %s failed, return %d", devpath, fd);
        goto err;
    }
    ...
    if (ret < 0) {
        ALOGE("rpaf compent algo set failed, %d", ret);
        goto err;
    }
err:
    if (fd > 0)
...
awrpaf_com_t *awrpaf_component_stream_init(int dsp_id)
{
    ...
    ALOGD("allwinner version:%s", MODULE_VERSION);
    ...
    fd = open(path, O_RDWR);
    if (fd < 0) {
        ALOGE("open %s failed, return %d", path, fd);
        goto err;
    }
    com->fd = fd;
    com->dsp_id = dsp_id;
...
int awrpaf_component_add_item(awrpaf_com_t *com, enum SND_DSP_COMPONENT_TYPE type)
{
    ...
    for (i = 0; i < ARRAY_SIZE(com->com_config.component_sort); i++) {
        if (com->com_config.component_sort[i] == type) {
            ALOGE("component type:%d already install.", type);
            return -1;
...
int awrpaf_component_stream_create(awrpaf_com_t *com, unsigned int len)
{
    ...
    xfer->result = 0;
    /*ALOGD("[%s] line:%d dump_buf=%p", __func__, __LINE__, xfer->dump_buf);*/

    ret = ioctl(com->fd, SND_SOC_DSP_COMPONENT_IOCTL_CREATE, &com->com_config);
    if (ret < 0) {
        ALOGE("rpaf compent create failed, %d", ret);
        goto err;
    }
    return ret;
err:
...
int awrpaf_component_stream_remove(awrpaf_com_t *com)
{
    ...
    if (ret != 0) {
        ALOGE("rpaf component remove failed, %d", ret);
        return ret;
    }
...
int awrpaf_component_set_buffer(awrpaf_com_t *com, unsigned int len)
{
    ...
    if (com->com_config.comp_mode == SND_DSP_COMPONENT_MODE_STREAM) {
        ...
        xfer->result = 0;
        /*ALOGD("[%s] line:%d dump_buf=%pn", __func__, __LINE__, xfer->dump_buf);*/
    } else {
        /*TODO*/
        xfer->dump_buf = malloc(len);
        if (!xfer->dump_buf) {
...
int awrpaf_component_dump(awrpaf_com_t *com, void **addr, ssize_t *len)
{
    ...
    *len = com->xferi.dump_length;
    if (ret < 0) {
        ALOGE("rpaf compent read failed, %d", ret);
        return -1;
    }
    *addr = com->xferi.dump_buf;
    *len = com->xferi.dump_length;
                             ...
                             void **output, ssize_t *output_len)
{
    ...
    if (input_len != com->xferi.input_length)
        ALOGE("input_len=%zd, %zd", input_len, com->xferi.input_length);
...
int awrpaf_component_stream_algo_list_set(awrpaf_com_t *com, int algos_type, int *algos_sort, int algo_num)
{
    ...
    if (algo_num > RPAF_COMPONENT_MAX_NUM)
        return -1;
    /*ALOGD("[%s] line:%d algo_num=%d", __func__, __LINE__, algo_num);*/
...
int awrpaf_component_stream_read(awrpaf_com_t *com, void *addr, ssize_t *len)
{
    ...
    ret = ioctl(com->fd, SND_SOC_DSP_COMPONENT_IOCTL_READ, &com->xferi);
    if (ret < 0) {
        ALOGE("rpaf component read failed, %d", ret);
        return ret;
    }
    /*ALOGD("[%s] line:%d len=%d, dump_len=%d, result=%d", __func__, __LINE__,*/
...
int awrpaf_component_stream_stop(awrpaf_com_t *com)
{
    ...
    if (ret < 0) {
        ALOGE("rpaf component stop failed, %d", ret);
        return ret;
    }
...
int awrpaf_component_independence_set_algo_params(awrpaf_com_t *com, void *params, unsigned int len)
{
    ...
    if (len > sizeof(pcm_params->algo_params)) {
        ALOGE("algo params size:%zd, but request %d",
...
/**
 * TODO:不知道上面的接口是干嘛的，重新定义一个接口传参
 * 该接口用于向内核设置算法参数
 * @params com
 * @params algo_type:算法类型
 * @params params:算法私有参数，不同算法的结构体不一样
 * @params len:params 的长度
 * @params is_changed:因 RPC 机制对传参长度有限制，有时候需要多次传参，当传参完毕需要生效时，该参数设置为true
 */
                                                   ...
                                                   int algo_type, void *params, unsigned int len, bool is_changed)
{
    ...
    if (len >= sizeof(algo_params.data)) {
        ALOGE("algo params len is to large: %d", len);
        return ret;
    }
    algo_params.algo_type = algo_type;
    algo_params.is_changed = is_changed;
    memcpy(algo_params.data, params, len);
    ret = ioctl(com->fd, SND_SOC_DSP_COMPONENT_IOCTL_ALGO_PARAMS_SET, &algo_params);
    return ret;
}
                                                  ...
                                                  void **export_input_buffer)
{
    ...
    *export_input_buffer = xfer->input_buf;
    ...
    if (ret < 0) {
        ALOGE("rpaf compent start failed, %d,%d", ret, status);
        return -1;
    }
                                          ...
                                          void **output, ssize_t *output_len)
{
    ...
    if (com->com_config.pcm_params.input_size < input_len ||
        ...
        ALOGE("input or output size is invalid.");
        return -1;
    }
    ...
    ret = ioctl(com->fd, SND_SOC_DSP_COMPONENT_IOCTL_WRITE, &com->xferi);
    if (ret < 0) {
        ALOGE("rpaf compent write failed, %d", ret);
        return -1;
    }
...
int awrpaf_component_independence_release(awrpaf_com_t *com)
{
    ...
    if (com->fd > 0) {
        ...
        if (ioctl(com->fd, SND_SOC_DSP_COMPONENT_IOCTL_STOP, &status) < 0)
            ALOGE("rpaf compent stop failed");
        if (ioctl(com->fd, SND_SOC_DSP_COMPONENT_IOCTL_REMOVE, &status) < 0)
            ALOGE("rpaf compent remove failed");
        close(com->fd);
        com->fd = -1;
    }
    if (com->xferi.output_buf)
