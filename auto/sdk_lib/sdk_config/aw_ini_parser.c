    ...
    char * value)
{
    ...
    if (len < 1) {
        ...
        /* Section name */
        sscanf(input_line, "[%32[^]]", section);
        strcpy(section, strstrip(section));
//        strcpy(section, strlwc(section));  /* Convert to lowercase. */
        sta = LINE_SECTION ;
    } else if (sscanf(input_line, "%32[^=] = \"%32[^\"]\"", key, value) == 2
               ||  sscanf(input_line, "%32[^=] = '%32[^\']'",   key, value) == 2
               ||  sscanf(input_line, "%32[^=] = %32[^;#]",     key, value) == 2) {
...
static int set_a_ini_value(const char *section, const char *key, const char *val)
{
    ...
    ret = confirm_location_of_key(ini_filep, section, key, &start);/* Muil-lines ????*/
    if (ret == -1) {
...
static int get_ini_value_type(const char *section, const char *key)
{
    ...
    UNUSED(num);
    if (strlen(buf) && (endp - buf) == (int)strlen(buf)) {
        type = VALUE_TYPE_NUM;
    } else if (strlen(buf)) {
        type = VALUE_TYPE_STR;
    } else {
