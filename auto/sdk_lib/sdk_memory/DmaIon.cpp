    ...
    if (0 == ret) {
        ...
        if (ret != 0) {
            ...
            if (pIommuBuffer->iommu_addr & 0xff) {
                ALOGE("get iommu addr maybe wrong:%x", pIommuBuffer->iommu_addr);
                return -1;
            }
            //ALOGD("ion_alloc_palloc: fd:%d, iommu_addr:%x",
...
int GetIonTotalMem()
{
    ...
    if (ret < 0) {
        ALOGE("Failed to ioctl ion device, errno:%s", strerror(errno));
        goto err;
    }

    ALOGD("ion dev get free pool [%d MB], total [%d MB]", binfo.free_mb, binfo.total / 1024);
    ret = binfo.total;
err:
    if (ion_fd > 0)
        close(ion_fd);
...
int IonAllocOpen()
{
    ...
    if (g_pAllocContext == NULL) {
        ...
        ALOGD("pid: %d, g_pAllocContext = %p ", getpid(), g_pAllocContext);
    }
...
int IonAllocClose()
{
    ...
    //
    if (!g_pAllocContext) {
        ALOGE("error:open and close dismatch.");
        return 0;
    }

    if (--g_pAllocContext->ref_cnt <= 0) {
        ALOGD("pid: %d, release g_pAllocContext = %p ", getpid(), g_pAllocContext);

        aw_mem_list_for_each_safe(pos, q, &g_pAllocContext->list) {
            tmp = aw_mem_list_entry(pos, ion_buffer_node, i_list);
            ALOGD("IonAllocClose del item phy = 0x%lx vir = 0x%lx, size = %d ", tmp->phy, tmp->vir, tmp->size);
            aw_mem_list_del(pos);
            free(tmp);
        }
        ...
        ALOGD("ref cnt: %d > 0, do not free ", g_pAllocContext->ref_cnt);
    }
    return 0;
}
               ...
               int* handle_fd)
{
    ...
    ALOGD("app cmd=%d", AW_ION_IOC_NEW_ALLOC);
    ret = ioctl(fd, AW_ION_IOC_NEW_ALLOC, &data);
    if (ret < 0) {
        ALOGE("new alloc fail");
        return -1;
    }
...
uint8_t *IonAlloc(int size)
{
    ...
    //ALOGD(" ===IonAlloc ");
    if (g_pAllocContext == NULL) {
        ALOGE("IonAlloc do not opened, should call ion_alloc_open() before ion_alloc(size) ");
        goto ALLOC_OUT;
    }
    ...
    if (0 == IonIsLegacy()) {
        ...
        /* mmap to user */
        addr_vir = (uint8_t*) mmap(NULL, alloc_data.aw_len, PROT_READ | PROT_WRITE, MAP_SHARED, fd_data.aw_fd, 0);
        if (MAP_FAILED == addr_vir) {
            ALOGE("mmap err, ret %d", addr_vir);
            addr_vir = 0;
            goto ALLOC_OUT;
        }
        ...
        if (ret) {
            ALOGE("ION_IOC_MAP err, ret %d, dmabuf fd 0x%08x", ret, (unsigned int)fd_data.aw_fd);
            goto ALLOC_OUT;
        }
        ...
        //ALOGD("fd_data.aw_fd %d", fd_data.aw_fd);
        ...
        if (ret) {
            ALOGE("ION_IOC_CUSTOM err, ret %d", ret);
            addr_phy = 0;
            addr_vir = 0;
            goto ALLOC_OUT;
        }
    ...
    if (alloc_buffer == NULL) {
        ...
        if (ret)
            ALOGDE"ION_IOC_FREE err, ret %d", ret);
#endif
        addr_phy = 0;
    ...
    //ALOGD("getPhyFromIommu gVefd(%d) aw_fd(%d)",gVefd,iommu_buffer.fd);
    ...
    //ALOGD("ionAlloc vir=0x%lx phy=0x%lx, size %d, fd %d", addr_vir, addr_phy,size, fd_data.aw_fd);
...
int IonFree(void *pbuf)
{
    ...
    aw_mem_list_for_each_entry(tmp, &g_pAllocContext->list, i_list) {
        if (tmp->vir == addr_vir) {
            ...
            //ALOGD("IonFree freePhyFromIommu gVefd(%d) aw_fd(%d)",gVefd,iommu_buffer.fd);
            ret = freePhyFromIommu(gVefd, (void *)&iommu_buffer);
            if (ret < 0) {
                ALOGE("free phy addr error");
            }
#endif

            //ALOGD("IonFree item phy= 0x%lx vir= 0x%lx, size= %d ", tmp->phy, tmp->vir, tmp->size);
            /*unmap user space*/
    ...
    if (0 == flag)
        ALOGE("IonFree failed, do not find virtual address: 0x%lx ", addr_vir);
    return nFreeSize;
}
...
int IonVir2fd(void *pbuf)
{
    ...
    aw_mem_list_for_each_entry(tmp, &g_pAllocContext->list, i_list) {
        if (addr_vir >= tmp->vir && addr_vir < tmp->vir + tmp->size) {
            ...
            //ALOGE("ion mem vir = %p, fd = %d", (void*)addr_vir, fd);
            flag = 1;
            break;
        }
    }
    ...
    if (0 == flag)
        ALOGE("IonVir2fd failed, do not find virtual address: 0x%lx ", addr_vir);
    ...
    if (1 == IonIsLegacy()) {
        aw_mem_list_for_each_entry(tmp, &g_pAllocContext->list, i_list) {
            if (addr_vir >= tmp->vir && addr_vir < tmp->vir + tmp->size) {
                ...
                //ALOGD("IonVir2phy tmp->phy%p + addr_vir%p - tmp->vir%p ",(void*)tmp->phy,(void*)addr_vir,(void*)tmp->vir);
                //ALOGD("IonVir2phy phy= 0x%p vir= 0x%p ", (void*)addr_phy, (void*)addr_vir);
                flag = 1;
                break;
            }
        }
        ...
        if (0 == flag)
            ALOGE("IonVir2phy failed, do not find virtual address: 0x%lx ", addr_vir);
...
uint8_t* IonPhy2vir(void *pbuf)
{
    ...
    if (1 == IonIsLegacy()) {
        ...
        if (0 == flag)
            ALOGE("IonPhy2vir failed, do not find physical address: 0x%lx ", addr_phy);
...
uint8_t* IonAllocDrm(int size)
{
    ...
    ret = ioctl(g_pAllocContext->fd, AW_MEM_ION_IOC_ALLOC, &alloc_data);
    if (ret) {
        ALOGE("ION_IOC_ALLOC error %s ", strerror(errno));
        goto ALLOC_OUT;
    }
    ...
    if (alloc_buffer == NULL) {
        ...
        if (ret)
            ALOGE("ION_IOC_FREE err, ret %d", ret);
