 ...
 *
 */
#define LOG_TAG "SoundControl_alsa"
#include <sdklog.h>
    ...
    if (sc->eStatus != SD_STATUS_STOPPED) {
        if (sc->alsa_handler) {
            ...
            if ((err = snd_pcm_close(sc->alsa_handler)) < 0) {
                LALOGW("__Release MSGTR_AO_ALSA_PcmCloseError");
            } else {
                sc->alsa_handler = NULL;
                LALOGW("__Release alsa-uninit: pcm closed\n");
            }
        } else {
            LALOGW("__Release MSGTR_AO_ALSA_NoHandlerDefined");
        }
    }
...
static void __SetFormat(SoundCtrl* s, CdxPlaybkCfg* cfg)//unsigned int nSampleRate, unsigned int nChannelNum)
{
    ...
    if (sc->eStatus != SD_STATUS_STOPPED) {
        LALOGE("Sound device not int stop status(%d), can not set audio params.", sc->eStatus);
        //pthread_mutex_unlock(&sc->mutex);
        //return;
    }
...
static int __Start(SoundCtrl* s)
{
    ...
    if (sc->eStatus == SD_STATUS_STARTED) {
        LALOGW("__Start Sound device already started.");
        pthread_mutex_unlock(&sc->mutex);
        return -1;
    }
    ...
    if (sc->eStatus == SD_STATUS_STOPPED) {
        ...
        LALOGV("__Start alsa-init: requested format: %d Hz, %d channels, %x\n", pcm_params.samplerate,
        ...
        LALOGD("__Start alsa-init: using device %s\n", alsa_device);
        ...
        if (!sc->alsa_handler) {
            if ((err = try_open_device(sc, alsa_device, sc->open_mode, 0)) < 0) {
                if (err != -EBUSY && sc->ao_noblock) {
                    LALOGE("MSGTR_AO_ALSA_OpenInNonblockModeFailed");
                    if ((err = try_open_device(sc, alsa_device, 0, 0)) < 0) {
                        pthread_mutex_unlock(&sc->mutex);
                        return 0;
                    }
                } else {
                    LALOGE("Playback open error: %s", snd_strerror(err));
                    pthread_mutex_unlock(&sc->mutex);
                    return 0;
                }
            }
            ...
            if ((err = snd_pcm_nonblock(sc->alsa_handler, 0)) < 0) {
                LALOGW("MSGTR_AO_ALSA_ErrorSetBlockMode");
            } else {
            ...
            if ((err = snd_pcm_hw_params_any(sc->alsa_handler, sc->alsa_hwparams)) < 0) {
                LALOGW("MSGTR_AO_ALSA_UnableToGetInitialParameters");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
            ...
            if (err < 0) {
                LALOGW("MSGTR_AO_ALSA_UnableToSetAccessType");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
            ...
            if ((err = snd_pcm_hw_params_set_channels_near(sc->alsa_handler,
                ...
                LALOGW("MSGTR_AO_ALSA_UnableToSetChannels");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
            ...
            if ((err = snd_pcm_hw_params_set_rate_resample(sc->alsa_handler, sc->alsa_hwparams,
                                                           0)) < 0) {
                LALOGW("MSGTR_AO_ALSA_UnableToDisableResampling");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
#endif
            ...
            if ((err = snd_pcm_hw_params_set_rate_near(sc->alsa_handler, sc->alsa_hwparams,
                ...
                LALOGW("MSGTR_AO_ALSA_UnableToSetSamplerate2");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
...
#ifdef BUFFERTIME
            {
                ...
                if ((err = snd_pcm_hw_params_set_buffer_time_near(sc->alsa_handler,
                    ...
                    LALOGW("MSGTR_AO_ALSA_UnableToSetBufferTimeNear");
                    pthread_mutex_unlock(&sc->mutex);
                    return 0;
                } else
...
#ifdef SET_CHUNKSIZE
            {
                ...
                if ((err = snd_pcm_hw_params_set_period_size_near(sc->alsa_handler,
                    ...
                    LALOGW("MSGTR_AO_ALSA_UnableToSetPeriodSize");
                    pthread_mutex_unlock(&sc->mutex);
                    return 0;
                } else {
                ...
                if ((err = snd_pcm_hw_params_set_periods_near(sc->alsa_handler,
                    ...
                    LALOGW("MSGTR_AO_ALSA_UnableToSetPeriods");
                    pthread_mutex_unlock(&sc->mutex);
                    return 0;
                } else {
            ...
            if ((err = snd_pcm_hw_params(sc->alsa_handler, sc->alsa_hwparams)) < 0) {
                LALOGW("MSGTR_AO_ALSA_UnableToSetHwParameters");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
            ...
            if ((err = snd_pcm_hw_params_get_buffer_size(sc->alsa_hwparams, &bufsize))
                < 0) {
                LALOGW("MSGTR_AO_ALSA_UnableToGetBufferSize");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            } else {
                pcm_params.buffersize = bufsize * sc->bytes_per_sample;
                LALOGW("alsa-init: got buffersize=%i\n", pcm_params.buffersize);
            }

            if ((err = snd_pcm_hw_params_get_period_size(sc->alsa_hwparams,
                                                         &sc->chunk_size, NULL)) < 0) {
                LALOGW("MSGTR_AO_ALSA_UnableToGetPeriodSize");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            } else {
                LALOGW("alsa-init: got period size %li\n", sc->chunk_size);
            }
            ...
            if ((err = snd_pcm_sw_params_current(sc->alsa_handler, sc->alsa_swparams)) < 0) {
                ...
                LALOGW("MSGTR_AO_ALSA_UnableToGetSwParameters");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
#if SND_LIB_VERSION >= 0x000901
            if ((err = snd_pcm_sw_params_get_boundary(sc->alsa_swparams, &boundary)) < 0) {
                LALOGW("MSGTR_AO_ALSA_UnableToGetBoundary");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
#else
            ...
            if ((err = snd_pcm_sw_params_set_start_threshold(sc->alsa_handler,
                ...
                LALOGW("MSGTR_AO_ALSA_UnableToSetStartThreshold");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
            ...
            if ((err = snd_pcm_sw_params_set_stop_threshold(sc->alsa_handler,
                ...
                LALOGW("MSGTR_AO_ALSA_UnableToSetStopThreshold");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
#if SND_LIB_VERSION >= 0x000901
            ...
            if ((err = snd_pcm_sw_params_set_silence_size(sc->alsa_handler,
                                                          sc->alsa_swparams, boundary)) < 0) {
                LALOGW("MSGTR_AO_ALSA_UnableToSetSilenceSize");
                pthread_mutex_unlock(&sc->mutex);
                return 0;
            }
#endif
            ...
            LALOGW("alsa: %d Hz/%d channels/%d bpf/%d bytes buffer\n",
        ...
        if (snd_pcm_state(sc->alsa_handler) == SND_PCM_STATE_SUSPENDED) { //it is right?
            LALOGW("MSGTR_AO_ALSA_PcmInSuspendModeTryingResume");
        ...
        if (0 && sc->alsa_can_pause) { //TODO fix
            if ((err = snd_pcm_pause(sc->alsa_handler, 0)) < 0) {
                LALOGW("MSGTR_AO_ALSA_PcmResumeError");
                pthread_mutex_unlock(&sc->mutex);
                return -1;
            }
            LALOGW("alsa-resume: resume supported by hardware\n");
        } else {
            if ((err = snd_pcm_prepare(sc->alsa_handler)) < 0) {
                LALOGW("MSGTR_AO_ALSA_PcmPrepareError");
                pthread_mutex_unlock(&sc->mutex);
                return -1;
            }
        }
...
static int SoundDeviceStop_l(SoundCtrlContext* sc)
{
    ...
    if (sc->eStatus == SD_STATUS_STOPPED) {
        LALOGW("Sound device already stopped.");
        return 0;
    }
    if (sc->alsa_handler == NULL) {
        LALOGW("SoundDeviceStop_l Sound device NULLL.");
    ...
    if (sc->eStatus != SD_STATUS_STOPPED) {
        if (sc->alsa_handler) {
            int err;
            if ((err = snd_pcm_close(sc->alsa_handler)) < 0) {
                LALOGW("MSGTR_AO_ALSA_PcmCloseError");
            } else {
                sc->alsa_handler = NULL;
                LALOGW("alsa-uninit: pcm closed\n");
            }
        } else {
            LALOGW("MSGTR_AO_ALSA_NoHandlerDefined");
        }
    ...
    if (0 && sc->alsa_can_pause) { //TODO fix
        if (sc->alsa_handler != NULL) {
            if ((err = snd_pcm_pause(sc->alsa_handler, 1)) < 0) {
                LALOGW("MSGTR_AO_ALSA_PcmPauseError");
                return -1;
            }
            LALOGW("alsa-pause: pause supported by hardware\n");
        }

    } else {
        if (sc->alsa_handler != NULL) {
            if ((err = snd_pcm_drop(sc->alsa_handler)) < 0) {
                LALOGW("MSGTR_AO_ALSA_PcmDropError");
            }
        }
    }
    LALOGW("sound __Pause");
...
static int __Write(SoundCtrl* s, void* pData, int nDataSize)
{
    ...
    LALOGD("alsa-play: frames=%i, len=%i\n", num_frames, nDataSize);
    if (!sc->alsa_handler) {
        LALOGW("MSGTR_AO_ALSA_DeviceConfigurationError");
        return 0;
    }
    ...
    do {
        ...
        if (res < 0) {
            LALOGW("MSGTR_AO_ALSA_WriteError");
...
{
    SoundCtrlContext* s;
    LALOGW("==snd   SoundDeviceInit");
    s = (SoundCtrlContext*)malloc(sizeof(SoundCtrlContext));
    if (s == NULL) {
        LALOGE("malloc memory fail.");
        return NULL;
    }
    ...
    if (hwRet == 255) {
        ALOGV("1111init chwd sucess !\r\n");
    } else {
        ALOGV("1111init chwd fail !\r\n");
        thisId = 5;
