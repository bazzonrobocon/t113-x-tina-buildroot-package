...
// 3.0.0_Beta
// float biases[24] = {169, 60, 105, 124, 67, 212, 145, 252, 252, 149,295, 297,
// 								8, 16, 13, 43, 40, 27, 23, 81, 62, 66, 38, 136};

// 4.0.0_Beta
float biases[24] = {58, 192, 273, 90, 100, 251, 161, 164, 179, 288,318, 288,
 								8, 14, 18, 33, 19, 77, 53, 49, 34, 122, 89, 102};

// yolo detection output layer
// input (batch, w,h,c); total is number of anchors
layer make_yolo_layer(int batch,int num,int w,int h,int n,int total,int classes)
{
    layer l = {0};
    l.n = n;
    l.total = total;
...
detection* get_detections(vector<blob> blobs,int img_w,int img_h, int netw, int neth, int *nboxes, int classes, float thresh, float nms)
{
    ...
    for(unsigned int i=0;i<blobs.size();++i){
        layer l_params = make_yolo_layer(1,blobs[0].w,blobs[i].w,blobs[i].h, 6, 12, classes);
    ...
    if(nms) do_nms_sort(dets,(*nboxes),classes,0.5);
    return dets;
}
