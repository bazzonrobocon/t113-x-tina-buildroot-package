...
{
    ALOGD("create AWVideoEncoderImpl() version:%s", MODULE_VERSION);
    afbcHeaderSize = 0;
...
int AWVideoEncoderImpl::init(EncodeParam* param, AWVideoEncoderDataCallback* cbk)
{
    ...
    if ((NULL != mEncodeParam) || (NULL != mDataCbk)) {
        ALOGE("The decoder has been initialized before.");
        return -1;
    }
    mEncodeParam = param;
    mDataCbk = cbk;
    ...
    if (baseConfig.memops == NULL) {
        ALOGD("MemAdapterGetOpsS failed");
        return -1;
    }
    ...
    if (ret) {
        ALOGE("setEncParam error, return");
        return -1;
    }
...
{
    //ALOGD("the average encode time is %lldus,file size:%d...", time3 / mFreamIndex);
...
int AWVideoEncoderImpl::writeOutputBuf(void* buf, int len)
{
    if (mOutputBufLen < mOutputBufPos + len) {
        ALOGE("writeOutputBuf out of range!!");
        return -1;
    }
    ...
    /*
       ...
       for (i = 0; i < num_bitMap; i++)
       {
           ...
           if (bitMapInfo[i].argbAddr == NULL)
           {
               ...
               if (bitMapInfo[i].argbAddr == NULL)
               {
                   ALOGE("malloc bitMapInfo[%d].argbAddr fail", i);
                   if (icon_hdle)
                   {
                       fclose(icon_hdle);
                       icon_hdle = NULL;
           ...
           ALOGV("bitMap[%d] size[%d,%d], size_align16[%d, %d], argbAddr:%p", i,
                bitMapInfo[i].width,
                bitMapInfo[i].height,
           ...
           if (ret != bitMapInfo[i].width * bitMapInfo[i].height * 4)
               ALOGE("read bitMap[%d] error, ret value:%d", i, ret);
       ...
       ALOGD("blk_num:%d, argb_type:%d", pOverlayInfo->blk_num, pOverlayInfo->argb_type);
       ...
       for (i = 0; i < pOverlayInfo->blk_num; i++)
       {
           ...
           ALOGV("blk_%d[%d], start_mb[%d,%d], end_mb[%d,%d],extra_alpha_flag:%d, extra_alpha:%d",
                i,
                time_id_list[i],
           ...
           ALOGV("overlay_type:%d, cover_yuv[%d,%d,%d], overlay_blk_addr:%p, bitmap_size:%d",
...
void AWVideoEncoderImpl::unInitOverlayInfo()
{
    /*
       ...
       ALOGD("sizeof(bitMapInfo)=%d", sizeof(bitMapInfo));*/
}
...
int AWVideoEncoderImpl::initH264Func(H264Func* h264Func, EncodeParam* encodeParam)
{
    ...
    ALOGD("h264Func->h264Param.sRcParam.eRcMode=%d", h264Func->h264Param.sRcParam.eRcMode);
...
int AWVideoEncoderImpl::initJpegFunc(JpegFunc* jpegFunc, EncodeParam* encodeParam)
{
    ...
    if (0 == jpegFunc->jpegMode)
        ...
        ALOGE("encoder do not support the jpegMode:%d", jpegFunc->jpegMode);
        return -1;
    }
...
int AWVideoEncoderImpl::setEncParam(VideoEncoder* pVideoEnc, EncodeParam* encodeParam)
{
    ...
    if (encodeParam->codecType == CODEC_JPEG) {
        ...
        if (result) {
            ALOGE("initJpegFunc error, return ");
            return -1;
        }
        ...
        if (result) {
            ALOGE("initH264Func error, return ");
            return -1;
        }
        ...
        if (result) {
            ALOGE("initH265Func error, return ");
            return -1;
        }
        unsigned int vbv_size = 12 * 1024 * 1024;
        VideoEncSetParameter(pVideoEnc, VENC_IndexParamSetVbvSize, &vbv_size);
...
void AWVideoEncoderImpl::initMbInfo(VencMBInfo* MBInfo, EncodeParam* encodeParam)
{
    ...
    if (MBInfo->p_para == NULL) {
        ALOGE("malloc MBInfo->p_para error");
        return;
    }
    //ALOGD("mb_num:%d, mb_info_queue_addr:%p", MBInfo->num_mb, MBInfo->p_para);
}
...
void AWVideoEncoderImpl::getMbMinfo(VideoEncoder* pVideoEnc, EncodeParam* encodeParam)
{
    ...
    for (i = 0; i < pMBInfo->num_mb; i++) {
        ALOGD("No.%d MB: mad=%d, qp=%d, sse=%d, psnr=%f", i, pMBInfo->p_para[i].mb_mad,
...
int AWVideoEncoderImpl::getHeader(void* spsHeader)
{
    ...
    if (mEncodeParam->codecType == CODEC_H264 || \
        ...
        //ALOGD("spsPpsData.nLength: %d", spsPpsData.nLength);
        //for(head_num=0; head_num<spsPpsData.nLength; head_num++)
        //ALOGD("the sps_pps :%02x", *(spsPpsData.pBuffer+head_num));
    }
}
...
int AWVideoEncoderImpl::setRoi(AvRoiRegionArray* roiRegions)
{
    ...
    if (NULL != roiRegions) {
        ALOGD("set roiRegions Num:%d ", roiRegions->roiNum);
        ...
        for (int k = 0; k < roiRegions->roiNum; k++) {
            ...
            CHECK_RETURN(ret);
            ALOGD("mRoiConfig[%d].bEnable      :%d", k, mRoiConfig[k].bEnable);
            ALOGD("mRoiConfig[%d].index        :%d", k, mRoiConfig[k].index);
            ALOGD("mRoiConfig[%d].nQPoffset    :%d", k, mRoiConfig[k].nQPoffset);
            ALOGD("mRoiConfig[%d].roi_abs_flag :%d", k, mRoiConfig[k].roi_abs_flag);
            ALOGD("mRoiConfig[%d].sRect.nLeft  :%d", k, mRoiConfig[k].sRect.nLeft);
            ALOGD("mRoiConfig[%d].sRect.nTop   :%d", k, mRoiConfig[k].sRect.nTop);
            ALOGD("mRoiConfig[%d].sRect.nWidth :%d", k, mRoiConfig[k].sRect.nWidth);
            ALOGD("mRoiConfig[%d].sRect.nHeight:%d", k, mRoiConfig[k].sRect.nHeight);
        }
    }
    pthread_mutex_unlock(&mEncodeMutex);
    return ret;
...
int AWVideoEncoderImpl::encode(AVPacket* inPacket)
{
    ...
    ALOGD("inputBuffer.addr:%lx->%lx ", inputBuffer.pAddrPhyY, inputBuffer.pAddrPhyC);

    VideoEncodeOneFrame(pVideoEnc);

    int validNum = ValidBitstreamFrameNum(pVideoEnc);
    //ALOGD("ValidBitstreamFrameNum is :%d.", validNum);
    if (validNum < 1) {
        ALOGW("ValidBitstreamFrameNum is 0.");
    }
    ...
    if (outPacket.id != inPacket->id) {
        ALOGD("change outPacket.id:%lx->%lx %lx", outPacket.id, inputBuffer.nID, inPacket->id);
        outPacket.id = inPacket->id;
    }
    if (outPacket.pts != inPacket->pts) {
        ALOGD("change outPacket.pts:%llx->%llx %llx", outPacket.pts, inputBuffer.nPts, inPacket->pts);
    ...
    if (mDataCbk) {
        ...
        ALOGE("AWVideoEncoderDataCallback is NULL.");
    }
...
//                }
    ...
    VideoEncodeOneFrame(pVideoEnc);
    //time2 = GetNowUs();
    //ALOGV("encode frame %d use time is %lldus..", mFreamIndex, (time2 - time1));
    //time3 += time2 - time1;
    ...
    if (mFreamIndex == (mEncodeParam->frameCount - 1)) {
        ...
        ALOGD("frame[%u] get min_psnr:%f, frame[%u] get max_psnr:%f, average_psnr:%f",
              maxSseFrame,
              min_psnr,
              minSseFrame,
              max_psnr,
    ...
    if ((sSuperFrameCfg.eSuperFrameMode == VENC_SUPERFRAME_DISCARD) && (result == -1)) {
        ALOGD("VENC_SUPERFRAME_DISCARD: discard frame %d", mFreamIndex);
        continue;
    }
#endif
    ...
    switch (TemporalLayer) {
        ...
        case 2:
        case 1:
            ALOGV("just write the PrefixNAL");
        ...
        default:
            ALOGV("\nTemporalLayer=%d,  mFreamIndex=%d", TemporalLayer, mFreamIndex);
