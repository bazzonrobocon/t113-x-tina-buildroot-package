...
{
    ALOGD("create AutAudioEnc() version:%s", MODULE_VERSION);
...
void AutAudioEnc::dropQueue()
{
    ...
    ALOGV("audio record dropQueue cnt %d sleep...", qcnt);
    if (qcnt > 0) {
        while (qcnt > 0) {
...
bool AutAudioEnc::AutAudioEncThread()
{
    ...
    if (mAudioThreadState == RECORD_STATE_STARTED) {
        ...
        if (ret == ERR_AUDIO_ENC_NONE) {
            ...
            // ALOGV("audio encode ERR_AUDIO_ENC_PCMUNDERFLOW");
            if (datmsg != NULL) {
                free(datmsg->extradata);
                free(datmsg);
            }
        ...
        if (pkmsg == NULL) {
            ALOGW("AutAudioEncThread malloc fail");
            if (datmsg != NULL) {
                free(datmsg->extradata);
                free(datmsg);
            }
        ...
        if (usrDatCb != NULL) {
            ...
            if (ret != 0) {
                ...
                if ((mAudioRecordQueueErr % 30) == 1) //FIRST AND 30ERROR REPORT
                    ALOGW("audio record queue full msgQSend %d", 1);
                free(pkt.buf);
                free(pkmsg);
            }
        } else {
...
{
    if (user == NULL) {
        ALOGW("__audioenc_data_src user null");
        return ;
    }
    ...
    if (datmsg == NULL) {
        ALOGW("__audioenc_data_src mesg fail");
        return ;
    }
    datmsg->extradata = (char *)malloc(dsize);
    if (datmsg->extradata == NULL) {
        ALOGW("__audioenc_data_src extradata fail");
        free(datmsg);
        return ;
    }
